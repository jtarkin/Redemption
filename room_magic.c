...
void spell_aura_of_peace( int sn, int level, CHAR_DATA *ch, void *vo, int target ) {
	AFFECT_DATA af;
	int caster_level, modified_level;
	...
	if (room_is_affected (ch->in_room, gsn_darkness))
	{
	    Cprintf(ch, "It can't get any more peaceful than this!\n\r");
	    return;
	}

	if(number_percent() < 20)
	{
		Cprintf(ch, "Your aura fails to appear.\n\r");
		return;
	}

    Cprintf(ch, "An aura of peace permeates the room.\n\r");
    act("An aura of peace permeates the room.", ch, NULL, NULL, TO_ROOM, POS_RESTING);
...
void
do_hideout(CHAR_DATA * ch, char * argument)
{
	AFFECT_DATA af, *paf;
	...
	if (is_affected(ch, gsn_hideout))
	{
		...
		if (ch->in_room->vnum == paf->modifier && paf != NULL) {
			paf->modifier = 0;
			paf->duration = UMIN(paf->duration, 5);
			Cprintf(ch, "You sadly say goodbye to your home.\n\r");
			paf = affect_find(ch->in_room->affected, gsn_hideout);
			affect_remove_room(ch->in_room, paf);
			return;
		} else if (paf->modifier == 0){
			Cprintf(ch, "You are still mourning the loss of your last hideout.\n\r");			
			
		} else {
			Cprintf(ch, "You have already built your hideout.\n\r");
			return;
		}
	}
	
	if (room_is_affected(ch->in_room, gsn_hideout)) {
		Cprintf(ch, "Another hermit already lives here.  Go find your own hideout.\n\r");
		return;
	}
	...
	if (ch->in_room == NULL
		...
		|| in_enemy_hall(ch))
	{
		Cprintf(ch, "You can't build a hideout here.\n\r");
		return;
	}
	
	if (ch->move < 50) {
		Cprintf(ch, "You don't have enough energy to spend it building a hideout.\n\r");
	}
	
	if (number_percent() < get_skill(ch, gsn_hideout))
	{
		check_improve(ch, gsn_hideout, TRUE, 5); //high multiplier because if they succeed, they have to wait a long time to try again
	
		/* now, set up affect, and add to player and room */
		af.where = TO_AFFECTS;
		af.type = gsn_hideout;
		af.level = ch->level;
		af.duration = ch->level * 2/ 3;
		af.location = 0;
		...
		Cprintf(ch, "You take the time to build a hideout in this room.\n\r");
		act("$n builds a hideout here.", ch, NULL, NULL, TO_ROOM, POS_RESTING);
		ch->move -= 50;
	} else {
		Cprintf(ch, "This place just doesn't feel like home, no matter how much you try.\n\r");
	}
}
