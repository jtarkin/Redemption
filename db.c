...
int gsn_divine_breath;
int gsn_wind_breath;
int gsn_water_breath;
int gsn_earth_breath;
int gsn_shadow_breath;

int gsn_brilliance;
int gsn_dominate;
int gsn_aura_of_peace;
int gsn_hibernate;
int gsn_harden;
...
int gsn_makegolem;
int gsn_destroy_magic;
int gsn_disassemble;
int gsn_infuse;
int gsn_repair_golem;
int gsn_wand_mastery;
int gsn_craft_mastery;

int gsn_create_ring;
int gsn_create_wand;
int gsn_create_staff;
int gsn_flux;
int gsn_fuse;
int gsn_graft;
int gsn_incinerate;
int gsn_create_forge;
int gsn_forge;
int gsn_golem_mastery;
int	gsn_toolbelt;
...
boot_db(void)
{
	...
	log_string("Reading the calendar...");
	load_time();
	...
	//This MUST come before area loading, as room/mob creation defaults with the
	//clan_lookup() function to find 'noclan'
	log_string("Scanning clan data...");
	read_clandata();
	 ...
	 */	
	{
		...
		snprintf( buf, MIL, "%s%s", AREA_DIR, AREA_LIST );
	 ...
	 */
	{
		...
		log_string("Refreshing all areas...");
		area_update();
...
//	read_clanleaders();
	...
	log_string("Filling empty containers...");
	load_persistent_containers();

	log_string("Filling shop shelves...");
	load_player_shops();
	
	log_string("Counting control areas...");
	clan_count_control_areas();
	return;
}
    ...
    for (;;) {
        ...
        switch (UPPER(word[0])) {
            case 'A':
                ...
                KEY("Autocontrol", pArea->autocontrol, fread_number(fp));
            case 'N':
		...
		if (!str_cmp(word, "Control"))
		{
			char* temp_str;

			temp_str = fread_string(fp);
			pArea->clan_control = clan_lookup(temp_str);
			...
			break;
                ...
                break;
	   case 'G':
		KEY("Gateable",pArea->gateable,fread_number(fp));
		break;
		case 'Q':
...
load_rooms(FILE * fp)
{
	...
	for (;;)
	{
		...
		pRoomIndex->clan = clan_lookup("noclan");
...
reset_room(ROOM_INDEX_DATA *pRoom, bool manualReset)
{
	...
	for (pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next)
	{
		...
		char buf[MAX_STRING_LENGTH];
		int count, limit = 0;
		int mobIsReset = 0; /*Used as boolean. Checks if the last mob loaded or not*/
		// Woot rare resets don't always kick in!
		if(pReset->percent > 0) {
			if(number_percent() > pReset->percent) {
				if (pReset->command == 'M') mobIsReset = 0;
				continue;
			}
			if (pReset->command == 'M') mobIsReset = 1;
		}
