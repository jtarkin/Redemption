...
DECLARE_DO_FUN(do_otimer);
DECLARE_DO_FUN(do_slookup);
DECLARE_DO_FUN(do_mload);
DECLARE_DO_FUN(do_cstring);
DECLARE_DO_FUN(do_ostring);
...
DECLARE_DO_FUN(do_geneqlist);
DECLARE_DO_FUN(do_vwhere);
DECLARE_DO_FUN(do_rewhere);
...
/* Finger command - under construction Apr 11 2009 -- Aql */
...
do_finger(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		Cprintf(ch, "You must specify a name.\n\r");
		return;
	}

	if ((test = get_char_world(ch, argument, TRUE)) != NULL && !IS_NPC(test))
	{
		if(ch->level > 53) {
			if (test->desc == NULL)
			{
				Cprintf(ch, "That person is linkdead or has been ploaded.\n\r");
			}
			else
			{
				if (!can_see(ch, test)) {
					Cprintf(ch, "%s's last login was within the past day.\n\r",capitalize(argument));
				} else {
			...
			if(test->level > 53) {
				Cprintf(ch, "%s's last login was in the not too distant future.\n\r",capitalize(argument));
				return;
			}
			else {
				Cprintf(ch, "%s's last login was within the past day.\n\r",capitalize(argument));
	...
	sprintf(dir, "%s%s", PLAYER_DIR, capitalize(argument));

	if(!(fp = fopen ( dir, "r" )))
	{
		Cprintf(ch, "Finger who? I cant seem to find them. Make sure to type out the full name!\n\r");
		return;
	}

	while (fgets(temp, MAXLINE, fp))
	{
		if (strncmp("LogO ", temp, 5) == 0)
		{
			found1 = TRUE;
			/* logon line - get it! */
			dump = strtok(temp," ");
		...
		if (strncmp("Levl ", temp, 5) == 0)
		{
			found2 = TRUE;
			/* level line - get it! */
			dump1 = strtok(temp, " ");
			dump1 = strtok(NULL, " ");
			if(atoi(dump1) > 53 && atoi(dump1) <= 60)
				is_imm = TRUE;
			else
				is_imm = FALSE;
		}
	...
	if(ch->level > 53) {
		Cprintf(ch, "%s's last login was %d days, %d hours, %d minutes and %d seconds ago.\n\r", capitalize(argument), d, h, m, s);
		return;
	}
	...
	if(d >= 365) {
		Cprintf(ch, "%s's last login was more than a year ago.\n\r",capitalize(argument));
		return;
	}
	else if(d >= 30) {
		Cprintf(ch, "%s's last login was within the last year.\n\r",capitalize(argument));
		return;
	}
	else if(d >= 7) {
		Cprintf(ch, "%s's last login was within the last month.\n\r",capitalize(argument));
		return;
	}
	else if(d >= 1) {
		Cprintf(ch, "%s's last login was within the last week.\n\r",capitalize(argument));
		return;
	}
	else {
		Cprintf(ch, "%s's last login was within the last day.\n\r",capitalize(argument));
		return;
	}
...
do_dice(CHAR_DATA * ch, char *argument)
{
	...
	Cprintf(ch, "Weapon: %dd%d -> Average: %f.\n\r", d1, d2, (1 + d2) * d1 / 2);
	return;
}
...
do_guild(CHAR_DATA * ch, char *argument)
{
	...
	if (get_trust(ch) == 52 || get_trust(ch) == 53){
		Cprintf(ch, "They are too important to their clan. Demote them first.\n\r");
		return;
	}
	...
	if (!str_prefix(arg2, "none"))
	{
		...
		set_clan(victim,"noclan");
		REMOVE_BIT(victim->wiznet, HARDCORE);
		victim->delegate = 0;
	...
	if (!found || clan == NULL)
	{
		Cprintf(ch,"That clan does not exist!\n\r");
		return;
	}
...
do_delegate(CHAR_DATA * ch, char *argument)
{
	...
	/*Make sure delegated player is online*/
	if ((victim = get_char_world(ch, arg1, TRUE)) == NULL)
	{
		Cprintf(ch, "They aren't playing.\n\r");
		return;
	...
	/*Make sure delegated player is not a mob*/
	if (IS_NPC(victim))
	{
	...
	/*Make sure the new delegate level is a number*/
	if (!is_number(arg2))
	{
		Cprintf(ch, "Syntax: delegate <char> <level>.\n\r");
		Cprintf(ch, "        delegate show\n\r");
	...
	/*Make sure delegating player is a leader*/
	if (get_trust(ch) < 53)
	{
	...
	/*Make sure delegated player is in the same clan*/
	...
	/*Make sure delegated player is a recruiter*/
	...
	/*Make sure that player cannot loner the leader or imms.*/
	if (level > 52)
	{
	...
	/*Make sure the delegate level is positive.*/
	if (level < 0)
	{
...
do_outfit(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->carry_number + 4 > can_carry_n(ch)) {
		Cprintf(ch, "You're too overburdened to petition the gods for more!\n\r");
		return;
	}
...
{
	Cprintf(ch,"Nochan now uses 'punish nochan'.\n\r");
	return;
}

void do_punish(CHAR_DATA *ch, char *argument)
{
	char arg[256], arg2[256], arg3[256], arg4[256], buf[256];
	CHAR_DATA *victim;
	int number = 0;
	int type = 0;
	int pen_type = 0;
	...
	if (arg[0] == '\0')
	{
		Cprintf(ch, "Syntax: punish <player> <pen type> [#] [hours/days/levels]");
		return;
	}
	...
	if (arg2[0] == '\0')
	{
		Cprintf(ch,"%s currently has the following active punishments:\n\r",victim->name);

		for (i = 0; i < MAX_PUNISH; i++)
		{
			if (victim->pcdata->punish_hoursleft[i] != 0)
			{

				if (i == 0)
				{
					if (victim->pcdata->punish_hoursleft[i] > 0)
						Cprintf(ch,"Frozen for %d hours.\n\r", victim->pcdata->punish_hoursleft[i]);
					else
						Cprintf(ch,"Frozen for unlimited hours.\n\r");
				}
				if (i == 1)
				{
					if (victim->pcdata->punish_hoursleft[i] > 0)
						Cprintf(ch,"Lagged for %d hours.\n\r", victim->pcdata->punish_hoursleft[i]);
					else
						Cprintf(ch,"Lagged for unlimited hours.\n\r");
				}
				if (i == 2)
				{
					if (victim->pcdata->punish_hoursleft[i] > 0)
						Cprintf(ch,"Nochanned for %d hours.\n\r", victim->pcdata->punish_hoursleft[i]);
					else
						Cprintf(ch,"Nochanned for unlimited hours.\n\r");
				}
				if (i == 3)
				{
					if (victim->pcdata->punish_hoursleft[i] > 0)
						Cprintf(ch,"Notelled for %d hours.\n\r", victim->pcdata->punish_hoursleft[i]);
					else
						Cprintf(ch,"Notelled for unlimited hours.\n\r");
				}
				if (i == 4)
				{
					if (victim->pcdata->punish_hoursleft[i] > 0)
						Cprintf(ch,"Nonoted for %d hours.\n\r", victim->pcdata->punish_hoursleft[i]);
					else
						Cprintf(ch,"Nonoted for unlimited hours.\n\r");
				}
				if (i == 5)
				{
					if (victim->pcdata->punish_hoursleft[i] > 0)
						Cprintf(ch,"Notitled for %d hours.\n\r", victim->pcdata->punish_hoursleft[i]);
					else
						Cprintf(ch,"Notitled for unlimited hours.\n\r");
				}				
			}	
			if (victim->pcdata->punish_daysleft[i] != 0)
			{

				if (i == 0)
				{
					if (victim->pcdata->punish_daysleft[i] > 0)
						Cprintf(ch,"Frozen for %d days.\n\r", victim->pcdata->punish_daysleft[i]/3600);
					else
						Cprintf(ch,"Frozen for unlimited days.\n\r");
				}
				if (i == 1)
				{
					if (victim->pcdata->punish_daysleft[i] > 0)
						Cprintf(ch,"Lagged for %d days.\n\r", victim->pcdata->punish_daysleft[i]/3600);
					else
						Cprintf(ch,"Lagged for unlimited days.\n\r");
				}
				if (i == 2)
				{
					if (victim->pcdata->punish_daysleft[i] > 0)
						Cprintf(ch,"Nochanned for %d days.\n\r", victim->pcdata->punish_daysleft[i]/3600);
					else
						Cprintf(ch,"Nochanned for unlimited days.\n\r");
				}
				if (i == 3)
				{
					if (victim->pcdata->punish_daysleft[i] > 0)
						Cprintf(ch,"Notelled for %d days.\n\r", victim->pcdata->punish_daysleft[i]/3600);
					else
						Cprintf(ch,"Notelled for unlimited days.\n\r");
				}
				if (i == 4)
				{
					if (victim->pcdata->punish_daysleft[i] > 0)
						Cprintf(ch,"Nonoted for %d days.\n\r", victim->pcdata->punish_daysleft[i]/3600);
					else
						Cprintf(ch,"Nonoted for unlimited daysn\r");
				}
				if (i == 5)
				{
					if (victim->pcdata->punish_daysleft[i] > 0)
						Cprintf(ch,"Notitled for %d days.\n\r", victim->pcdata->punish_daysleft[i]/3600);
					else
						Cprintf(ch,"Notitled for unlimited days.\n\r");
				}				
			}	
			if (victim->pcdata->punish_levelsleft[i] != 0)
			{

				if (i == 0)
				{
					if (victim->pcdata->punish_levelsleft[i] > 0)
						Cprintf(ch,"Frozen for %d levels.\n\r", victim->pcdata->punish_levelsleft[i]);
					else
						Cprintf(ch,"Frozen for unlimited levels.\n\r");
				}
				if (i == 1)
				{
					if (victim->pcdata->punish_levelsleft[i] > 0)
						Cprintf(ch,"Lagged for %d levels.\n\r", victim->pcdata->punish_levelsleft[i]);
					else
						Cprintf(ch,"Lagged for unlimited levels.\n\r");
				}
				if (i == 2)
				{
					if (victim->pcdata->punish_levelsleft[i] > 0)
						Cprintf(ch,"Nochanned for %d levels.\n\r", victim->pcdata->punish_levelsleft[i]);
					else
						Cprintf(ch,"Nochanned for unlimited levels.\n\r");
				}
				if (i == 3)
				{
					if (victim->pcdata->punish_levelsleft[i] > 0)
						Cprintf(ch,"Notelled for %d levels.\n\r", victim->pcdata->punish_levelsleft[i]);
					else
						Cprintf(ch,"Notelled for unlimited levels.\n\r");
				}
				if (i == 4)
				{
					if (victim->pcdata->punish_levelsleft[i] > 0)
						Cprintf(ch,"Nonoted for %d levels.\n\r", victim->pcdata->punish_levelsleft[i]);
					else
						Cprintf(ch,"Nonoted for unlimited levels.\n\r");
				}
				if (i == 5)
				{
					if (victim->pcdata->punish_levelsleft[i] > 0)
						Cprintf(ch,"Notitled for %d levels.\n\r", victim->pcdata->punish_levelsleft[i]);
					else
						Cprintf(ch,"Notitled for unlimited levels.\n\r");
				}				
			}				
		}
	...
	if (arg3[0] != '\0' && arg4[0] != '\0' && is_number(arg3))
	{
		number = atoi(arg3);
		if (!str_prefix(arg4,"hours")) type = 1;
		else if (!str_prefix(arg4,"days")) type = 2;
		else if (!str_prefix(arg4,"levels")) type = 3;		
		else
		{
			Cprintf(ch,"You must choose hours, days, or levels.");
			return;
		}
	}
	else
	...
	if (!str_prefix(arg2,"nochannel"))
	{
		pen_type = PUNISH_NOCHAN;			
	...
	else if (!str_prefix(arg2,"lag"))
	{
		if (get_trust(ch) >= 56) {
			...
			Cprintf(ch, "You don't have permission to do that yet.\n\r");
			return;
		}
	}
	else if (!str_prefix(arg2,"freeze"))
	{
		if (get_trust(ch) >= 56) {
			pen_type = PUNISH_FREEZE;			
			...
			else
			{
				...
				Cprintf(victim, "The ice creeps over your being!  You can't do ANYthing!\n\r");
	...
	else if (!str_prefix(arg2,"nonote"))
	{
		pen_type = PUNISH_NONOTE;			
	...
	else if (!str_prefix(arg2,"notitle"))
	{
		pen_type = PUNISH_NOTITLE;			
	...
	{
		Cprintf(ch,"That is not a valid punishment type.\n\t");
		return;
	}
	if (type == 1)   	{
		victim->pcdata->punish_hoursleft[pen_type] = number;
		Cprintf(ch, "Punishment applies for %d hours.\n\r", number);
	} else if (type == 2) {
		victim->pcdata->punish_daysleft[pen_type] = number*3600;
		Cprintf(ch, "Punishment applies for %d days.\n\r", number);
	} else if (type == 3) {
		victim->pcdata->punish_levelsleft[pen_type] = number;		
		Cprintf(ch, "Punishment applies for %d levels.\n\r", number);
	}
	else
	{
		if (!removed) Cprintf(ch,"No punishment duration set, punishment set to unlimited hours (until removed).\n\r");
		victim->pcdata->punish_daysleft[pen_type] = 0;		
		victim->pcdata->punish_hoursleft[pen_type] = -1;
		victim->pcdata->punish_levelsleft[pen_type] = 0;
	}
	if (removed)
	{
		victim->pcdata->punish_daysleft[pen_type] = 0;		
		victim->pcdata->punish_hoursleft[pen_type] = 0;
		victim->pcdata->punish_levelsleft[pen_type] = 0;
	}
	return;
}
...
{
	Cprintf(ch,"Nonote now uses 'punish nonote'.\n\r");
...
//Created 21/05/09 at 42000 ft in the air
// Split from do_bamfin
// - Avacar
void
do_logingreet(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch))
	{
		...
		if (arg1[0] == '\0')
		{
			Cprintf(ch, "Possible options are: send, show and set");
			return;
		} else if (!str_cmp("show", arg1))	{
			if (ch->pcdata->logingreet[0] != '\0') {
				Cprintf(ch, "Your welcome message is currently: \"%s\"\n\r", ch->pcdata->logingreet);
			} else {
				Cprintf(ch, "You do not have a welcome message set yet.\n\r");
			}
			return;
		} else if (!str_cmp("set", arg1))	{
			free_string(ch->pcdata->logingreet);
			ch->pcdata->logingreet = str_dup(argument);
			Cprintf(ch, "Your welcome message is now: \"%s\"\n\r", ch->pcdata->logingreet);
			return;
		} else if (!str_cmp("send", arg1)) 	{
			if (ch->pcdata->logingreet[0] != '\0') {
				strcpy(buf, ch->pcdata->logingreet);
			} else {
				strcpy(buf, "You hear a low rumbling in the distance.");
			};
			do_echo(ch, buf);
			return;
		}	else	{
...
//Created 21/05/09 at 42000 ft in the air
// copied from do_logingreet
// - Avacar
void
do_logoutgreet(CHAR_DATA * ch, char *argument)
{
	char arg1[MAX_STRING_LENGTH], buf[MAX_STRING_LENGTH];
	...
	if (!IS_NPC(ch))
	{
		...
		if (arg1[0] == '\0')
		{
			...
			{
				Cprintf(ch, "Your goodbye message is currently: \"%s\"\n\r", ch->pcdata->logoutgreet);
			} else {
				Cprintf(ch, "You don't have a goodbye message set yet. \n\r");
			}
			return;
		} else 	if (!str_cmp("set", arg1))	{
			free_string(ch->pcdata->logoutgreet);
			ch->pcdata->logoutgreet = str_dup(argument);
			Cprintf(ch, "Your goodbye message is now: \"%s\"\n\r", ch->pcdata->logoutgreet);
			return;
		} else 	if (!str_cmp("send", arg1))	{
			if (ch->pcdata->logoutgreet[0] != '\0') {
				strcpy(buf, ch->pcdata->logoutgreet);
			} else {
				strcpy(buf, "The world around you grows silent as an immortal departs.");
			}
			do_echo(ch, buf);
			return;
		} else {
...
do_goto(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch) && ch->level < 55 && get_trust(ch) < 55 && !IS_BUILDER(ch, location->area))
	{
...
do_ostat(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->level >= 56 && obj->creator != NULL )
		Cprintf(ch, "{cCreator:{x %s\n\r", obj->creator);
	else if (ch->level >= 56 && obj->creator == NULL)
		Cprintf(ch,"{cCreator:{x {RNULL!!{x",obj->creator);

	Cprintf(ch, "{cVersion:{x %d\n\r", obj->version);
	...
	Cprintf(ch, "{cIW Level:{x %d %d %d %d %d %d %d \n\r",
          obj->iw_level[0],
          obj->iw_level[1],
          obj->iw_level[2],
          obj->iw_level[3],
	...
	case ITEM_THROWING:
		Cprintf(ch, "{cDice:{x %d {cFaces:{x %d {cDamtype:{x %s {cSpell lvl:{x %d {cSpell:{x '%s'\n\r", obj->value[0], obj->value[1], attack_table[obj->value[2]].name, obj->value[3], skill_table[obj->value[4]].name);
		break;
...
do_mstat(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(victim))
	{
        ...
        if(ch->pcdata->email != NULL && ch->pcdata->email[0] != '\0') {
            ...
            Cprintf(ch, "{c%-*s{x %-*s\n\r",
                name_col, "E-Mail: ",
                col, victim->pcdata->email);
        }
	    if ( is_dragon( victim ) )
			Cprintf( ch, "{c%-*s{x %d/%d, Raw: %d\n\r",
		    name_col, "Breaths:",
		    victim->breath / 10, get_dragon_breaths( victim ) / 10, victim->breath );
	...
	if (victim->res_flags)
	{ // why reinvent the wheel?  this works in olc quite nicely
	  // and only requires updating the table if something changes.
		Cprintf(ch, "{c%-*s{x %s\n\r",
				name_col, "Res:",
	...
	if (victim->shop_mob_vnum > 0 && !IS_NPC(victim))
		Cprintf(ch,"Shopkeeper Mob Index: %d\n\r",victim->shop_mob_vnum);
	if (victim->is_player_shop > 0 && IS_NPC(victim))
		Cprintf(ch,"This mobile is a Player Shop.\n\r");
	return;
}
...
do_ofind(CHAR_DATA * ch, char *argument)
{
	 ...
	 * -- Furey
	 */
	Cprintf(ch, "Searching for objects...\n\r");
...
passpass(CHAR_DATA * ch, char *characterName, char *newPassword)
{
	...
	pwdnew = (char *) long_crypt(newPassword, charFilename);
	for (p = pwdnew; *p != '\0'; p++)
	{
		if (*p == '~')
		{
	 ...
	 */
	if (system(temp) != 0) fprintf(stderr, "System call failed in passpass: temp.\n");
...
do_setpass(CHAR_DATA * ch, char *argument)
{
	...
	{
		pwdnew = (char *) long_crypt(argument, victim->name);
		for (p = pwdnew; *p != '\0'; p++)
		{
			if (*p == '~')
			{
...
void do_vwhere(CHAR_DATA *ch, char *argument) {
	AREA_DATA *pArea;
	int vnum;

	if (argument == NULL || argument[0] == '\0')
	{
		Cprintf(ch, "You need to specify a vnum to locate.\n\r");
		return;
	}
	if (!is_number(argument)) {
		Cprintf(ch, "SYNTAX: vwhere <vnum>\n\r");
		return;
	}
	vnum = atoi(argument);
	...
	for (pArea = area_first; pArea; pArea = pArea->next) {
		if (pArea->min_vnum <= vnum && pArea->max_vnum >= vnum) {
			Cprintf(ch, "Results for VNum %d:\n\r"
					    ...
					    vnum,
					    pArea->name,
					    (pArea->continent==0)?"Terra":"Dominia",
					    pArea->min_vnum,
					    pArea->max_vnum,
					    (pArea->security==9)?"True":"False",
					    (pArea->questable==TRUE)?"True":"False",
					    (pArea->gateable==TRUE)?"True":"False",
					    (pArea->autocontrol==TRUE)?"True":"False");
			return;
		}
	}
	Cprintf(ch, "No areas contain that vnum.\n\r");
	return;
}
...
do_owhere(CHAR_DATA * ch, char *argument)
{
	...
	for (obj = object_list; obj != NULL; obj = obj->next)
	{
		...
		&& (ch->level > 55 || get_trust(ch) > 55 || IS_BUILDER(ch, in_obj->carried_by->in_room->area)))
		...
		&& (ch->level > 55 || get_trust(ch) > 55 || IS_BUILDER(ch, in_obj->in_room->area)))
...
do_mwhere(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		if (ch->level < 56 && get_trust(ch) < 56)
		{
...
do_reboot(CHAR_DATA * ch, char *argument)
{
	...
	for (d = descriptor_list; d != NULL; d = d_next)
	{
		...
		if(vch != NULL) {
			...
			for (i = 0; i < MAX_PUNISH; i++)
			{
				if (vch->pcdata->punish_daysleft[i] > 0)
				{
					vch->pcdata->punish_daysleft[i]-= (current_time - ch->logon)/24;
					if (vch->pcdata->punish_daysleft[i] == 0)
						finish_punish(vch,i);
				}
			}
...
void /* Coded by Aql on Apr 11 2009 */
do_oclone(CHAR_DATA * ch, char *argument)
{
	char buf[MAX_STRING_LENGTH];
...
void /* Coded by Aql on Apr 11 2009 */
do_mclone(CHAR_DATA * ch, char *argument)
{
	CHAR_DATA *clone = '\0';
	OBJ_DATA *new_obj;
	CHAR_DATA *mob;
...
void /* Modified by Aql on Apr 11 2009 */
...
do_purge(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		...
		if (!IS_NPC(ch) && ch->level < 55 && get_trust(ch) < 55 && !IS_BUILDER(ch, ch->in_room->area))
		{
	...
	if (!IS_NPC(victim))
	{
		...
		if (ch->level < 55 && get_trust(ch) < 55)
		{
			Cprintf(ch, "You failed.\n\r");
			return;
		}
...
do_vapourize(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->level < 55 && get_trust(ch) < 55 && !IS_BUILDER(ch, ch->in_room->area))
	{
		Cprintf(ch, "Builders can only purge rooms in their area.\n\r");
		return;
	}
...
do_advance(CHAR_DATA * ch, char *argument) {
	...
	if (level == 52 || level == 53)
	{
		Cprintf(ch,"You can no longer advance people to 52 or 53. Please use clanedit.\n\r");
		return;
	}
	...
	if (level <= victim->level)
	{
		...
		Cprintf(victim, "**** You have been demoted to level %d ****\n\r", level);
	...
	Cprintf(victim, "You have been promoted to level %d.\n\r", victim->level);
	victim->trust = 0;
	victim->delegate = 0;
	save_char_obj(victim, FALSE);
	return;
...
restore_char(CHAR_DATA * ch)
{
	...
	//affect_strip(ch, gsn_shifting_sands); this spell was replaced with acid_rain
	...
	ch->move = MAX_MOVE(ch);
	age = get_age(ch);
	//usion check here for dragon breath
	if ( is_dragon( ch ) )
	    ch->breath = get_dragon_breaths( ch );
	else
	    ch->breath = 0; // sanity -K
	ch->can_lay = 0;
...
{
	Cprintf(ch,"Freeze now uses 'punish freeze'.\n\r");
	return;
}
...
{
	Cprintf(ch,"Notitle now uses 'punish notitle'.\n\r");
	return;
}
...
do_set(CHAR_DATA * ch, char *argument)
{
	...
	{
		Cprintf(ch,"Set clans is obsolete. Use clanedit.\n\r");
		//		do_cset(ch, argument);
		return;
	}
...
//now completely obsolete, use clanedit -- Teldin
//function kept because I'm too lazy to remove it from the command list
...
do_mset(CHAR_DATA * ch, char *argument) {
	...
	if ((arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0') && arg1[0] != '?') {
		...
		Cprintf(ch, "    plimit remort reclass claims email\n\r");
	...
	if (arg1[0] == '?') {
		...
		add_buf(buffer, buf);
		sprintf(buf, "     drunk:    sets drunkenness on PCs.  Range: -1 to 100.\n\r");
		add_buf(buffer, buf);
		sprintf(buf, "     email:    sets email address for the owner of this PC.");
		add_buf(buffer, buf);
		...
		add_buf(buffer, buf);
		sprintf(buf, "     Claims:   sets the number of claims remaining on a Ruthless player.\n\r");
		add_buf(buffer, buf);
	...
	if (!str_prefix(arg2, "email")) {
	    ...
	    if (strlen(arg3) >= 80)
		{
			Cprintf(ch, "Sorry, email must be less than 80 characters.\n\r");
			return;
		}
		else
		{
			...
			if (ch != victim) {
			    Cprintf(ch, "E-mail address for %s is now:\n\r %s\n\r", victim->name, victim->pcdata->email);
			}
			Cprintf(victim, "E-mail address for this character is now:\n\r  %s\n\r", victim->pcdata->email);
            return;
		}
	}
	...
	if (!str_prefix(arg2, "security")) {
		...
		if (ch->level < 60 && get_trust(ch) < 60) {
	...
	if (!str_prefix(arg2, "drunk")) {
		...
		if (ch != victim) {
			Cprintf(ch, "%s's drunkenness set to %d.\n\r", vicName, value);
		}

		Cprintf(victim, "Your drunkenness set to %d.\n\r", value);
	...
	if (!str_prefix(arg2, "claims")) {
		victim->pcdata->claims_remaining = value;
		Cprintf(victim, "Your remaining claims has been set to %d.\n\r", value);
		Cprintf(ch, "%s's remaining available claims has been set to %d.  Recall this\n\r", victim->name, value);
		Cprintf(ch, "only matters for Ruthless characters.\n\r");
		return;
	}
...
void /* Coded by Aql on Apr 11 2009 */
do_cstring(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
	{
		...
		{
			if (ch->level == 58)
				Cprintf(ch, "Resetting character whodesc.\n\r");
			else
...
void /* Coded by Aql on Apr 11 2009 */
do_ostring(CHAR_DATA * ch, char *argument)
{
...
do_string(CHAR_DATA * ch, char *argument)
{
	...
	{
		do_cstring(ch,argument);
	}
	else if (!str_prefix(type, "object"))
	{
		do_ostring(ch,argument);
	}
	else
	{
		Cprintf(ch, "Syntax:\n\r");
...
do_lag(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET(victim->comm, COMM_LAG))
	{
		...
		sprintf(buf, "$N distracts the LAG monster from %s.", victim->name);
		wiznet(buf, ch, NULL, WIZ_PENALTIES, 0, 0);
		victim->pcdata->punish_hoursleft[PUNISH_LAG] = 0;
		victim->pcdata->punish_daysleft[PUNISH_LAG] = 0;
		victim->pcdata->punish_levelsleft[PUNISH_LAG] = 0;
	}
	else
	{
		SET_BIT(victim->comm, COMM_LAG);
		...
		sprintf(buf, "$N sics the LAG beast on %s.", victim->name);
		wiznet(buf, ch, NULL, WIZ_PENALTIES, 0, 0);
		victim->pcdata->punish_hoursleft[PUNISH_LAG] = -1;
		victim->pcdata->punish_daysleft[PUNISH_LAG] = 0;
		victim->pcdata->punish_levelsleft[PUNISH_LAG] = 0;		
	}
...
do_join(CHAR_DATA * ch, char *argument)
{
	...
	{
		Cprintf(ch, "You are not experienced enough to clan. Get to level 10.\n\r");
		return;
	}
	...
	//In a clan already
	if (((!ch->clan->loner) && ch->clan->clanner) || ((!ch->clan->clanner) && (ch->clan != clan_lookup("noclan"))))
	{
		Cprintf(ch, "You cannot join a new clan while you owe you allegiance elsewhere!\n\r");
		Cprintf(ch, "You must loner first.\n\r");		
		return;
	}

	if (clan->clanner != ch->clan->clanner)
	{
	...
	if (get_trust(ch) > 51)
	{
		Cprintf(ch,"You cannot join a clan if you are in leadership. Drop the position first.\n\r");
		return;
	}
...
do_accept(CHAR_DATA * ch, char *argument)
{
	...
	if (get_trust(ch) < 52)
	{
		Cprintf(ch,"You aren't leadership!\n\r");
		return;
	}

	if (victim->join_clan != NULL && victim->join_clan == ch->clan)
	{
		if (in_own_hall(victim))
		{
...
void do_pdouble(CHAR_DATA * ch, char *argument)
{
	char arg1[MAX_STRING_LENGTH], disp_msg[MAX_STRING_LENGTH], buf[MAX_STRING_LENGTH];
	int toggle;
	...
	/* Open/set up the boot file */

	if (!(fp = fopen (bootfile,"r+")))
	{
		//log_string("No boot file found. Creating.");
		fp = fopen (bootfile,"w+");
		fputs("BOOT_DOUBLE_XP 0\n",fp);
		fputs("BOOT_DOUBLE_QP 0\n",fp);
		fclose (fp);
		fp = fopen (bootfile, "r+");
	}
	argument = one_argument(argument, arg1);
	...
	if (arg1[0] == '\0')
	{
		Cprintf(ch, "Possible things to pdouble are: xp, qp.\n\r");
		return;
	}
	...
	{		
		...
		Cprintf(ch, "Perma-double %s disabled.\n\r", disp_msg);
		strcpy(dump,"BOOT_DOUBLE_");
		strcat(dump,(!str_cmp("xp",arg1)?"XP ":"QP "));
		while (fgets(temp, MAXLINE, fp)) 
		{
			if (strncmp(dump,temp, 14) == 0)
			{
				strcat(dump,"0");
		...
		sprintf(buf, "Double %s turned off :(", disp_msg);
		do_echo(ch, buf);
	}
	else
	{
		Cprintf(ch, "Perma-double %s enabled.\n\r", disp_msg);
		strcpy(dump,"BOOT_DOUBLE_");
		strcat(dump,(!str_cmp("xp",arg1)?"XP ":"QP "));
		while (fgets(temp, MAXLINE, fp)) 
		{
		...
		sprintf(buf, "Now doubling %s.", disp_msg);
		do_echo(ch, buf);
	}
	fclose(fp);
}
...
{
	char arg1[MAX_STRING_LENGTH], disp_msg[MAX_STRING_LENGTH], buf[MAX_STRING_LENGTH];
	int length; // this was "bool toggle", now int length
	//DESCRIPTOR_DATA *d;
	...
	if (arg1[0] == '\0')
	{
		Cprintf(ch, "Possible things to double are: xp, qp.\n\r  Type double status for current status.\n\r");
		return;
	}

	//old code was a toggle, now just take the value as a number
	/*if (argument[0] == '\0')
		toggle = 0;
	else
		toggle = 1;*/

	if(!str_cmp("quest", argument) && !str_cmp("xp", arg1))
	{
		length = QUEST_DOUBLE_XP_TICKS;
	}
	else if(!str_cmp("quest", argument) && !str_cmp("qp", arg1))
	{
		length = QUEST_DOUBLE_QP_TICKS;
	}
	else if(!str_cmp("remort", argument) && !str_cmp("xp", arg1))
	{
		length = REMORT_DOUBLE_XP_TICKS;
	}
	else
	{
		length = atoi(argument);
	}
	...
	if (!str_cmp("xp", arg1))
	{
		if (double_xp_ticks == -1) {
			length = -1;
			Cprintf(ch, "XP is already permanently on!  Use pdouble to turn off before setting a timer!");
		} else {	
			double_xp_ticks = length;
			strcpy(disp_msg, "experience points");
		}
	...
	else if (!str_cmp("qp", arg1))
	{
		if (double_qp_ticks == -1) {
			length = -1;
			Cprintf(ch, "QP is already permanently on!  Use pdouble to turn off before setting a timer!");
		} else {	
			double_qp_ticks = length;
			strcpy(disp_msg, "quest points");
		}


	}
	else if(!str_cmp("status", arg1))
	{
		length = -1;  //necessary to prevent the double is off messages below; doesn't matter since we've already set it
		if (double_xp_ticks > 0) {
			Cprintf(ch, "Double xp on for %d ticks\n\r", double_xp_ticks);

		} else if (double_xp_ticks == -1) {
			Cprintf(ch, "Double xp on permanently; use pdouble to turn off.\n\r");

		} else {
			Cprintf(ch, "Double xp is off.\n\r");

		}
		if (double_qp_ticks > 0) {
			Cprintf(ch, "Double qp on for %d ticks\n\r", double_qp_ticks);

		} else if (double_qp_ticks == -1) {
			Cprintf(ch, "Double qp on permanently; use pdouble to turn off.\n\r");

		} else {
			Cprintf(ch, "Double qp is off.\n\r");
	...
	if ((length == 0) && (length != -1) && str_cmp("quest", argument) && str_cmp("remort", argument))
	{
	...
	else if ((length > 0) && (length != -1) && str_cmp("quest", argument) && str_cmp("remort", argument))
	{
		...
		/*for (d = descriptor_list; d; d = d->next)
		{
			...
			{
				Cprintf(d->character, "Now doubling %s.\n\r", disp_msg);
				Cprintf(d->character, "Now doubling %s for %u ticks.\n\r", disp_msg, length);
			}
		}
		}*/
		sprintf(buf, "Now doubling %s for %u ticks.", disp_msg, length);
		do_echo(ch, buf);
	}
}
...

{
	...
	DESCRIPTOR_DATA *d;
	OBJ_DATA *obj = NULL;
	OBJ_DATA *obj_in = NULL;

	argument = one_argument(argument, arg);

	if (arg[0] == '\0')
	{
		Cprintf(ch, "List of Ruthless players online:\n\r---------------------------------\n\r\n\r");

		for (d = descriptor_list; d != NULL; d = d->next)
		{
			if (d->character != NULL)
			{
				if ((IS_SET(d->character->wiznet, HARDCORE))
						&& !IS_IMMORTAL(d->character)
						&& can_see(ch, d->character)) {
		...
		if (!str_cmp("status", argument)) {
			if (IS_HARDCORE(victim)) {
				Cprintf(ch, "They are hardcore!\n\r");
			} else {
				Cprintf(ch, "they are not hardcore.\n\r");
			}
			return;	
		}
		...
		if (!is_clan(victim))
		{
			Cprintf(ch, "They have to be part of the clan system.");
			return;
		}

		if ((get_trust(victim) >= get_trust(ch)) && (strcmp(victim->name, ch->name) != 0))
		{
			Cprintf(ch, "Be careful or they might smite you.\n\r");
			return;
		}
		...
		{
			REMOVE_BIT(victim->wiznet, HARDCORE);
			Cprintf(victim, "You are now a wimp.\n\r");
			Cprintf(ch, "Hardcore status removed.\n\r");
			sprintf(buf, "$N removes %s's hardcore status.", victim->name);
			wiznet(buf, ch, NULL, WIZ_FLAGS, 0, 0);
			log_string(buf);
		}
		else
		{
			SET_BIT(victim->wiznet, HARDCORE);
			Cprintf(victim, "You are now RUTHLESS!\n\r");
			Cprintf(victim, "Warning: all your claimed items are now unclaimed.  You may only claim %d\n\r", MAX_HK_CLAIMS);
			Cprintf(victim, "items in total.  So make your selections.");	
			Cprintf(ch, "Hardcore status set.\n\r");
			sprintf(buf, "$N grants %s ruthless status.", victim->name);
			wiznet(buf, ch, NULL, WIZ_FLAGS, 0, 0);
			log_string(buf);

			// strip the claim status off their items so they can choose which to keep, and reset their claim number
			/* Cautionary note: We're only checking the claimed items they have.  If they've been looted recently, they might
			be able to exceed their claim limit.  Given this reset check only happens on an imm command though, they have to know
			their cheating for this to occur, so nail'em. */

			for(obj = victim->carrying; obj != NULL; obj = obj->next_content) {
				if(obj->contains) {
					for(obj_in = obj->contains; obj_in != NULL; obj_in = obj_in->next_content) {
						if(!str_cmp(obj_in->respawn_owner, victim->name))
							free_string(obj_in->respawn_owner);
						obj_in->respawn_owner = NULL;
					}
				}

				if(!str_cmp(obj->respawn_owner, victim->name)) {
					free_string(obj->respawn_owner);
					obj->respawn_owner = NULL;
				}
			}
			victim->pcdata->claims_remaining = MAX_HK_CLAIMS;
		}
		...
		Cprintf(ch, "Type ruthless on its own for a list of ruthless players.\n\r");
		return;
	}
}
...
do_swapleader(CHAR_DATA * ch, char *argument)
{
	...
	/*Mobs and non-leaders cannot swap*/
	...
	/*Make sure the new leader is online*/
	...
	/*new leader must be in the same clan*/
	...
	/*Make sure that the target is not the leader.*/
	if (victim->level == 52)
	{

		/*If the leader is not the one in the clan data, demote him*/
		if (!str_cmp(ch->name, ch->clan->leader)) {
			demote_leader(ch);
			Cprintf(ch, "You are not the true leader! You've been demoted!\n\r");
			return;
		}

		if (!str_cmp(victim->name, victim->clan->leader)) {
			demote_leader(victim);
			Cprintf(victim, "You are not the true leader! You've been demoted!\n\r");
		}
	}

	/*if the new leader is a recruiter, demote them first*/
	demote_recruiter(victim);
	/*Replace the old leader with the new one.*/
	demote_leader(ch);
...
do_droprecruiter(CHAR_DATA * ch, char *argument)
{
	...
	/*Mobs and non-leaders cannot droprecruiter*/
	if (IS_NPC(ch) || get_trust(ch) != 53)
	{
	...
	/*Make sure the recruiter is online*/
	if ((victim = get_char_world(ch, arg2, TRUE)) == NULL)
	{
		Cprintf(ch, "You can't find them.\n\r");
		return;
	...
	/*Cannot drop yourself*/
	if (victim == ch)
	{
		Cprintf(ch, "Huh? Demote yourself?\n\r");
		return;
	}

	/*Make sure the recruiter is in the same clan*/
	if (victim->clan != ch->clan)
	{
		Cprintf(ch, "They aren't even in your clan!\n\r");
		return;
	...
	/*Make sure victim is a recruiter*/
	if (get_trust(victim) != 52)
	{
		Cprintf(ch,"They aren't a recruiter!\n\r");
		return;
	}

	/*demote the recruiter*/
	demote_recruiter(victim);
...
do_addrecruiter(CHAR_DATA * ch, char *argument)
{
	...
	/*Mobs and non-leaders cannot add recruiters*/
	if (IS_NPC(ch) || get_trust(ch) != 53)
	{
		Cprintf(ch, "You're not the leader of a clan!\n\r");
		return;
	}
	...
	/*Make sure new recruiter exists*/
	...
	/*Make sure new recruiter is present*/
	if (victim->clan != ch->clan)
	{
		Cprintf(ch, "They aren't even in your clan!\n\r");
		return;
	...
	/*Make sure there is a free recruiter slot*/
	if (str_cmp("none",ch->clan->recruiter1)
			&& str_cmp("none",ch->clan->recruiter2))
	{
		Cprintf(ch,"You have two recruiters already! You must drop one.\n\r");
		return;
	}

	/*Make sure that they are not already a recruiter*/
	if (get_trust(victim) >= 52)
	{
		Cprintf(ch,"They are already a recruiter!\n\r");
		return;
	}

	/*Set the new recruiter*/
...
demote_recruiter(CHAR_DATA * ch)
{
	...
	/*Make sure the person is a recruiter for their clan.*/
	if (str_cmp(ch->name, ch->clan->recruiter1) &&  str_cmp(ch->name, ch->clan->recruiter2)) {
		if ((get_trust(ch) == 52)) {
			/*If they are not supposed to be a recruiter but have the level anyway - demote them.*/
			ch->trust = 0;
			ch->delegate = 0;
		}
		return;
	}
	if (get_trust(ch) != 52) return;

	/*Reset trust and delegation status*/
	ch->trust = 0;
	ch->delegate = 0;

	/*Remove them from whichever recruiter slot they occupy*/
	if (!str_cmp(ch->name,ch->clan->recruiter1) )
	{
		free_string(ch->clan->recruiter1);
		ch->clan->recruiter1 = str_dup("none");
	}
	else
	{
		free_string(ch->clan->recruiter2);
		ch->clan->recruiter2 =str_dup("none");
	}
...
demote_leader(CHAR_DATA * ch)
{
	...
	/*Make sure he is actually the leader of the clan*/
	if (str_cmp(ch->name, ch->clan->leader)) {
		/*Check if he is a leader but shouldn't be and demote if necessary*/
		if (ch->level == 52) {
			ch->level -= 1;
			ch->max_hit -= 20;
			ch->max_mana -= 20;
		}
		/*reduce trust of non leader if necessary*/
		if (get_trust(ch) == 53) {
			ch->trust = 0;
			ch->delegate = 0;
		}
	...
	/*If 52 then reduce level back to 51*/
	if (ch->level == 52)
	{
		ch->level -= 1;
		ch->max_hit -= 20;
	...
	/*Remove trust and delegation*/
	ch->trust = 0;
	ch->delegate = 0;

	/*Remove leader from clandata*/
	free_string(ch->clan->leader);
	ch->clan->leader = str_dup("none");
...
advance_recruiter(CHAR_DATA * ch)
{
	...
	/*Set the first free recruiter slot to the new recruiter*/
	if (!str_cmp("none",ch->clan->recruiter1))
	{
		free_string(ch->clan->recruiter1);
		ch->clan->recruiter1 = str_dup(ch->name);
	}
	else
	{
		free_string(ch->clan->recruiter2);
		ch->clan->recruiter2 = str_dup(ch->name);				
	}
...
{
	/*Ensure new leader exists*/
	if (ch == NULL) return;

	/*Make sure that clan requires a leader*/
	if (str_cmp(ch->clan->leader,"none")) return;

	/*Set the new leaders name in clandata*/
	free_string(ch->clan->leader);
	ch->clan->leader = str_dup(ch->name);

	/**If the leader is level 51, advance to 52*/
	if (ch->level == 51) {
		ch->level++;
		ch->max_hit += 20;
		ch->max_mana += 20;
...
do_loner(CHAR_DATA * ch, char *argument)
{
	...
	// NC's can't loner
	if (ch->clan->loner || !ch->clan->clanner)
	{
	...
	else
	{
		...
		if (get_trust(ch) == 52 && ch->delegate < get_trust(victim))
		{
		...
		if (get_trust(victim) >= 52)
		{
			Cprintf(ch, "You cannot loner leadership.\n\r");
			return;
		}

		if (ch == victim)
		{
			Cprintf(ch, "You can't loner yourself!\n\r");
			return;
		}
	...
	else 
	{
		...
		Cprintf(victim, "You are *SO* banished.\n\r");
	}

	/* Check the clanreport to see if they're on there. If so, ditch them. */

	if (!str_cmp(victim->name,victim->clan->mostkills))
	{
		free_string(victim->clan->mostkills);
		victim->clan->mostkills = str_dup("none");
		victim->clan->numkills = 0;
	}
	if (!str_cmp(victim->name,victim->clan->mostkilled))
	{
		free_string(victim->clan->mostkilled);
		victim->clan->mostkilled = str_dup("none");
		victim->clan->numkilled = 0;		
	}

	if (victim->clan->clanner)
	{
		victim->clan = clan_lookup("outcast");
		victim->outcast_timer = time_outcast;
	}
	else
		victim->clan = clan_lookup("noclan");
...
do_resetpassword(CHAR_DATA * ch, char *argument)
{
	...
	pwdnew = (char *) long_crypt(arg2, victim->name);
	for (pwdchk = pwdnew; pwdchk[0] != '\0'; pwdchk++) {
		if (pwdchk[0] == '~') {
			Cprintf(ch, "Unacceptable password. Choose a different password.\n\r");
			return;
		}
	}
	free_string(victim->pcdata->pwd);
   ...
   if (victim->was_in_room != NULL)    /* return player and pet to orig room */
    {
        ...
        if (victim->pet != NULL)
        {
            ...
            char_to_room(victim->pet, victim->pet->was_in_room);
        }
    }
...
void advance_craft(CHAR_DATA *gch, OBJ_DATA *obj)
{
	...
	if (gch->in_room->clan != clan_lookup("noclan")) //for crafting in clan halls
		return;
	...
	if (gch->charClass == class_lookup("artificer") &&
			gch->reclass == reclass_lookup("battlesmith"))
		xp /= 5; //battlesmith penalty
...
void do_craft(CHAR_DATA *ch, char *argument) 
{
		...
		if (is_name(pObjIndex->name, arg)) {
			...
			while(TRUE) {
				...
				if (ch->charClass == class_lookup("artificer") &&
						ch->reclass == reclass_lookup("battlesmith"))
				{
					if (get_obj_qty(ch,partIndex) < UMAX(part->qty/2,1))
					{
						missingItems = TRUE;
						break;
					}
	...
	chance += get_skill(ch, gsn_craft_mastery)/4;
	chance -= (pObjIndex->level + 25);     

	if (get_skill(ch,gsn_craft_mastery) < number_percent())
	{
		if(number_percent() > chance) {
			Cprintf(ch, "You try and craft an item, but your skills fail you!\n\r");
			Cprintf(ch, "You will have to try again in 15 minutes.\n\r");
			ch->craft_timer = 15;
			if (get_skill(ch,gsn_craft_mastery) > 0)    
				check_improve(ch,gsn_craft_mastery,FALSE,1);		    
			return;
		}
	}

	if (get_skill(ch,gsn_craft_mastery) > 0)
		check_improve(ch,gsn_craft_mastery,1,TRUE);
...
vote_set(CHAR_DATA *ch, char *argument)
{
	...
	if(field[0] == '\0') {
		Cprintf(ch, "Vote set fields: question 1 2 3 4 5 level\n\r");
		return;
	}
	if(value[0] == '\0') {
...
void save_vote() {
	...
	fptr = fopen("../area/voteinfo.txt", "wb");
...
void load_vote() {
	...
	fptr = fopen("../area/voteinfo.txt", "rb");
	...
	if (fread(&vote, sizeof(vote), 1, fptr) == 0) fprintf(stderr, "Fread call failed in load_vote.\n");
	fclose(fptr);
	return;
}
...
void fixgroups(CHAR_DATA *ch) {
	bool all_known;
	bool found;
	int counter = 0;

	found = TRUE;
	// log_string("fixgroups: Starting while loop.");
	while ( found && counter < 10 ) {
		...
		for (gn = 0; gn < MAX_GROUP; gn++) {
			...
			for (sn = 0; sn < MAX_IN_GROUP; sn++) {
				...
				// log_string("fixgroups: In for loop for group [%s].", group_table[gn].name);

				int skill_id = skill_lookup(group_table[gn].spells[sn]);
				int group_id = group_lookup(group_table[gn].spells[sn]);

				if (skill_id > -1) {
					if (ch->pcdata->learned[skill_id] == 0) {
						all_known = FALSE;
						break;
					}
				}
				...
				if (group_id > -1) {
					if (ch->pcdata->group_known[group_id] == 0) {
						all_known = FALSE;
						break;
					}
				}
	...
	// log_string("fixgroups: Died after %i loops.", counter);
}
...
do_clan_rank(CHAR_DATA * ch, char *argument) {
			...
			for (d = descriptor_list; d != NULL; d = d->next) {
						...
						&& d->character->clan == clan)
				{
					if (d->character->clan_rank) {
						...
						found = TRUE;
						Cprintf(ch, formatstring, d->character->name, d->character->clan_rank, capitalize(d->character->clan->name));
					} else {
						found = TRUE;
						Cprintf(ch, "%-15s  %-25s  %-10s\n\r", d->character->name, d->character->clan_rank ? d->character->clan_rank : "Member", capitalize(d->character->clan->name));
					}
				}
			}
		}
	...
	if (!ch->clan || get_trust(ch) < 52  || ch->clan->loner )
	{
...
do_member_list(CHAR_DATA * ch, char *argument) {
	...
	clan = clan_lookup(clan_name);

	/* Not an existing clan name?! */
	if (!str_cmp(clan->name,"noclan") || clan== NULL )
	{
	...
	if (system(grep_input) != 0) fprintf(stderr, "System call failed in do_member_list: grep_input\n");
	...
	fclose(grep_file);
	if (system("rm grep_output.dat") != 0) fprintf(stderr, "System call failed in do_member_list: rm grep_output.dat\n");
...
do_pledge(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->clan->clanner != vch->clan->clanner)
	{
		Cprintf(ch, "You cannot pledge to someone in a different clan type.\n\r");
		return;
	}
	...
	if (IS_IMMORTAL(ch) && ch->level >= 55)
	{
		...
		if (!strcmp(arg1, "end"))
		{
			for(fch = char_list; fch != NULL; fch = fch->next)
			{
				...
				{
					char_from_room(fch);
					char_to_room(fch, get_room_index(fch->clan->spawn_terra));
...
void finish_punish(CHAR_DATA *ch, int punish_type)
{
	if (ch == NULL || IS_NPC(ch)) return;

	Cprintf(ch,"Your punishment time is up!\n\r");

	if (punish_type == PUNISH_NOCHAN)
	{
		REMOVE_BIT(ch->comm, COMM_NOCHANNELS);
		Cprintf(ch, "The gods have restored your channel privileges.\n\r");
		wiznet("$N's nochan automatically removed.", ch, NULL, WIZ_PENALTIES, 0, 0);
	}
	if (punish_type == PUNISH_LAG)
	{	
		REMOVE_BIT(ch->comm, COMM_LAG);
		Cprintf(ch, "The LAG monster has decided to prey upon some other fool.\n\r");
		wiznet("$N's lag automatically removed.", ch, NULL, WIZ_PENALTIES, 0, 0);
	}
	if (punish_type == PUNISH_FREEZE)
	{		
		REMOVE_BIT(ch->act, PLR_FREEZE);
		Cprintf(ch, "You can play again.\n\r");
		wiznet("$N's freeze automatically removed.", ch, NULL, WIZ_PENALTIES, 0, 0);
	}
	if (punish_type == PUNISH_NONOTE)
	{
		REMOVE_BIT(ch->comm, COMM_NONOTE);
		Cprintf(ch, "The gods have restored your note-writing abilities.\n\r");
		wiznet("$N's nonote automatically removed.", ch, NULL, WIZ_PENALTIES, 0, 0);
	}
	if (punish_type == PUNISH_NOTITLE)
	{
		Cprintf(ch, "The gods reinstated your right to express yourself.\n\r");
		REMOVE_BIT(ch->toggles, TOGGLES_NOTITLE);
		wiznet("$N's notitle automatically removed.", ch, NULL, WIZ_PENALTIES, 0, 0);
	}
	return;
}
void do_geneqlist(CHAR_DATA *ch, char *argument){
	char arg[256];
	int out = 0;
	Cprintf(ch,"%d\n\r", top_obj_index);
	if ((get_trust(ch) != 60 && ch->level != 60) || ch->pcdata->security < 9) {
		Cprintf(ch,"Insufficient security to waste resources generating an eqlist.\n\r");
		return;
	}
	argument = one_argument(argument, arg);
	if (strcmp(arg, "flatfile") == 0) {
		/*flat file format, or comma separated value table.
		 * a plain text file with one record per line with
		 * fields separated by a specific character. Because
		 * tildes cannot be used, the tilde character is the
		 * one chosen in this case.
		 */
		out = 0;
	}else{
		Cprintf(ch,"Invalid file format.\n\r"
				"SYNTAX: geneqlist <format>\n\r"
				"The current available formats are:\n\r"
				"flatfile\n\r"
				"Game will automatically reboot when eqlist has been created.\n\r");
		return;
	}
	Cprintf(ch,"Beginning generation of eq list...\n\r");
	generate_eqlist(out);
	Cprintf(ch,"Eq list complete. Rebooting.\n\r");
	/*if (!read_from_ident(d->ifd, buffer) || IS_NULLSTR(buffer)) {
	 * The preeceeding line in comm.c process_ident will freeze for
	 * unknown reasons after using this command, so a reboot is
	 * requires.*/
	reboot_all();
	return;
}

void do_timechange(CHAR_DATA *ch, char *argument)
{
	char arg[256];
	int setting;
	...
	if (arg[0] == '\0' || argument[0] == '\0')
	{
		Cprintf(ch,"Change what? Day, month, or year?\n\r");
		return;
	}
	if (!is_number(argument))
	{
		Cprintf(ch,"The setting must be a number.\n\r");
		return;
	}
	setting = atoi(argument);
	if (!str_prefix(arg,"day"))
	{
		if (setting < 0 || setting > 34)
		{
			Cprintf(ch,"Day must be between 0 and 34.");
			return;
		}
		time_info.day = setting;
		Cprintf(ch,"Game day has been modified.\n\r");
		save_time();
		return;
	}
	if (!str_prefix(arg,"month"))
	{
		if (setting < 0 || setting > 16)
		{
			Cprintf(ch,"Month must be between 0 and 16.");
			return;
		}
		time_info.month = setting;
		Cprintf(ch,"Game month has been modified.\n\r");
		save_time();
		return;
	}
	if (!str_prefix(arg,"year"))
	{
		if (setting < 1 || setting > 24000)
		{
			Cprintf(ch,"Year must be between 1 and 24k.");
			return;
		}
		time_info.year = setting;
		Cprintf(ch,"Game year has been modified.\n\r");
		save_time();
		return;
	}	
	return;
...
void do_set_sliver_type(CHAR_DATA *ch, char *argument)
{
	CHAR_DATA *victim;
	char arg[MAX_INPUT_LENGTH];
	char arg2[MAX_INPUT_LENGTH];
	...
	if (arg[0] == '\0' || arg2[0] == '\0')
	{
		Cprintf(ch,"Syntax: setsliver <player> <type>\n\r");
		return;
	}
	for (slivertype = 1; sliver_table[slivertype].name != NULL; slivertype++)
	{
		if (!str_prefix(arg2,sliver_table[slivertype].name))
		{
			found = TRUE;
			break;
		}
		if (slivertype > 20)
		{
			bug("Error in do_set_sliver_type: infinite loop!",0);
			return;
		}
	}
	if (!found)
	{
		Cprintf(ch,"That sliver type does not exist. See Help Sliver.\n\r");
		return;
	}
	if ((victim = get_char_world(ch, arg, TRUE)) == NULL)
	{
	...
	if (IS_NPC(victim))
	{
		Cprintf(ch,"Not on mobs.\n\r");
		return;
	}
	if (victim->race != race_lookup("sliver"))
	{
		Cprintf(ch,"The target has to be a sliver!\n\r");
		return;
	}
	ch->sliver = slivertype;
	act("$n morphs into a new kind of sliver!", ch, NULL, NULL, TO_ROOM, POS_RESTING);
	Cprintf(victim, "You are morphed into a %s sliver!\n\r", sliver_table[ch->sliver].name);
	act("You change $N into a new type of sliver.",ch,NULL,victim,TO_CHAR,POS_RESTING);
	return;
}

void
do_otimer(CHAR_DATA * ch, char *argument)
{
	int timer = 0;
	OBJ_DATA *obj;
	char object[MAX_INPUT_LENGTH], time[MAX_INPUT_LENGTH];
	argument = one_argument(argument, object);
	argument = one_argument(argument, time);

	if (object[0] == '\0' || time[0] == '\0')
	{
		Cprintf(ch, "Syntax:\n\r");
		Cprintf(ch, "  otimer <object> <time>\n\r");
		return;
	}

	if (get_trust(ch) < 56) {
		Cprintf(ch, "You try to set up a timer, but even an hourglass boggles your mind.\n\r");
		return;
	}

	/* string an obj */
	if ((obj = get_obj_world(ch, object)) == NULL)
	{
		Cprintf(ch, "Nothing like that in heaven or earth.\n\r");
		return;
	}
	...
	if (!is_number(time))
	{
		Cprintf(ch, "Time is relative, but timers are not. You must put in an integral value.\n\r");
		return;
	}
	...
	if (timer < 0) {
		Cprintf(ch, "This is a positive mud, please give a positive time.\n\r");
		return;
	}

	if (timer == 0) {
		if ((obj->pIndexData->extra_flags & ITEM_ROT_DEATH) && get_trust(ch) < 58) {
			Cprintf(ch, "You are not yet powerful enough to preserve this item.\n\r");
			return;
		}
		obj->extra_flags &= !ITEM_ROT_DEATH;
		obj->timer = 0;
		Cprintf(ch, "Your powerful magics preserve %s from the ravages of time.\n\r", obj->short_descr);
		act("$n envelops $p in a powerful aura of preservation!", ch, obj, NULL, TO_ROOM, POS_RESTING);
		return;
	}

	obj->extra_flags |= ITEM_ROT_DEATH;
	obj->timer = timer;
	Cprintf(ch, "Your powerful magics corrode %s, slowly degrading it.\n\r", obj->short_descr);
	act("$n envelops $p in a putrid aura of corruption!", ch, obj, NULL, TO_ROOM, POS_RESTING);
	return;
}
...
void do_powerlevel( CHAR_DATA* ch, char* argument )
{
    ...
    if ( !obj )
    {
    Cprintf( ch, "Powerlevel what?\n\r" );
    return;
    }
    if ( obj->item_type != ITEM_WEAPON
      || !IS_WEAPON_STAT(obj, WEAPON_INTELLIGENT) )
    {
    act( "$p cannot be powerleveled.", ch, obj, NULL, TO_CHAR, POS_RESTING );
    return;
    }
    xp = atoi( argument );
    if ( xp <= 0 )
    {
    Cprintf( ch, "Positive xp values only.\n\r" );
    return;
    }
    advance_weapon( ch, obj, xp );
    Cprintf( ch, "%s has been powerleveled with %d xp.\n\r", obj->short_descr, xp );
    return;
}

void
do_rewhere(CHAR_DATA * ch, char *argument)
{
	OBJ_INDEX_DATA *obj, *in_obj;
	MOB_INDEX_DATA *mob;
	ROOM_INDEX_DATA *room;
	RESET_DATA *reset;
	int is_vnum = 0, found = 0, vnum, i;

	//Must have access to area
	if (!IS_BUILDER(ch, ch->in_room->area)) {
		Cprintf(ch, "Insufficient security to modify resets.\n\r");
		return;
	}

	//Must specify a name or vnum to search for
	if (argument[0] == '\0') {
		Cprintf(ch, "What reset were you looking for?\n\r");
		return;
	}

	//If argument is a number search for resets by vnum
	if (is_number(argument))
	{
		is_vnum = 1;
		vnum = atoi(argument);
	...
	//Check each room in the current area... imms should use vnum and vwhere to find correct location first.
		...
		for (reset = room->reset_first; reset != NULL; reset = reset->next) {
			...
			case 'M':
				if (is_vnum) {
					...
					mob = get_mob_index(reset->arg1);
					if (!mob || !is_name(argument, mob->player_name))
						continue;
				}
				found++;
				Cprintf(ch, "Mob [%d] \"%s\" at [%d] \"%s\"\n\r", mob->vnum, mob->short_descr, room->vnum, room->name);
				break;
			//Objects in room
			case 'O':
				if (is_vnum) {
				...
				found++;
				Cprintf(ch, "Obj [%d] \"%s\" at [%d] \"%s\"\n\r", obj->vnum, obj->short_descr, room->vnum, room->name);
				break;
			...
			case 'P':
				...
				in_obj = get_obj_index(reset->arg3);
				if (!in_obj)
					continue;
				found++;
				Cprintf(ch, "Obj [%d] \"%s\" in [%d] \"%s\" at [%d] \"%s\"\n\r", obj->vnum, obj->short_descr, in_obj->vnum, in_obj->short_descr, room->vnum, room->name);
				break;
			//Carried by Mob
			case 'G':
			//Equipped by Mob
			case 'E':
				...
				if (!mob)
					continue;
				found++;
				Cprintf(ch, "Obj [%d] \"%s\" in [%d] \"%s\" at [%d] \"%s\"\n\r", obj->vnum, obj->short_descr, mob->vnum, mob->short_descr, room->vnum, room->name);
				break;
			}
		}
	}
	...
	if (!found) {
		Cprintf(ch, "No resets found with a %s matching %s\n\r", (is_vnum)?"vnum":"name", argument);
		return;
	}
	Cprintf(ch, "%d resets found with a %s matching %s\n\r", found, (is_vnum)?"vnum":"name", argument);
	return;
}
