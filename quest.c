...
bool has_bank_note(CHAR_DATA * ch);
OBJ_DATA *find_bank_note(CHAR_DATA * ch, OBJ_DATA * obj);

/* for hunt quests */
int hunt_quest_timer = 0;
int hunt_continent;
int h1;
int h2;
int h3;
char hint1[MAX_STRING_LENGTH];
char hint2[MAX_STRING_LENGTH];
char hint3[MAX_STRING_LENGTH];

/* Local functions */
void generate_quest(CHAR_DATA *, CHAR_DATA *, int);
bool chance(int num);
ROOM_INDEX_DATA *find_location(CHAR_DATA * ch, char *arg);
void gear20 (CHAR_DATA *);
...
void quest_info(CHAR_DATA *ch)
{
	...
	if(hunt_quest_timer == -1) {
		Cprintf(ch, "{xThere is currently a hunt on for a rare and wondrous beast!\n\r\n\r");
		
		if(hint1[0] == '\0')
			Cprintf(ch, "No hints have been given yet.\n\r\n\r");
		else {
			Cprintf(ch, "The last %shint%s:\n\r\n\r",
						hint3[0] != '\0' ? "three " : (hint2[0] != '\0' ? "two " : ""),
						hint3[0] != '\0' ? "s were" : (hint2[0] != '\0' ? "s were" : " was"));
			Cprintf(ch, "{g1.{x %s\n\r", hint1);
			if(hint2[0] != '\0')
				Cprintf(ch, "{g2.{x %s\n\r", hint2);
			if(hint3[0] != '\0')
...
void quest_reset(CHAR_DATA *ch)
{
	...
	ch->pcdata->quest.timer = ch->pcdata->quest.timer * 0.666;
...
void quest_list(CHAR_DATA *ch, CHAR_DATA *questmaster)
{
	...
	for (obj = questmaster->carrying; obj != NULL; obj = obj->next_content) {
		if (IS_SET(obj->wear_flags, ITEM_QUEST)) {
			Cprintf(ch, "%-4dqp ........ %s\n\r", obj->cost, obj->name);
		}
	}
	/*
...
void quest_buy(CHAR_DATA *ch, CHAR_DATA *questmaster, char *argument)
{
	...
	for (obj = questmaster->carrying; obj != NULL; obj = obj->next_content) {
		if (IS_SET(obj->wear_flags, ITEM_QUEST) && is_name(argument, obj->name)) {
			Cprintf(ch, "You ask the questmaster to trade %s for %-4dqp.\n\r", obj->short_descr, obj->cost);
			buy_obj = obj;
			break;
		}
	}
	...
	if (buy_obj == NULL) {
		sprintf(buf, "I don't have any %s, %s.", argument, ch->name);
		do_say(questmaster, buf);
		return;
	}

	if (ch->questpoints < obj->cost) {
		sprintf(buf, "You need %d quest points to buy a %s, %s.",obj->cost, argument, ch->name);
		do_say(questmaster, buf);
		return;
	}

	ch->questpoints -= obj->cost;
	buy_obj = create_object(obj->pIndexData, obj->level);
	act("$N gives $p to $n.", ch, buy_obj, questmaster, TO_ROOM, POS_RESTING);
	act("$N gives you $p.", ch, buy_obj, questmaster, TO_CHAR, POS_RESTING);
	obj_to_char(buy_obj, ch);

	free_string(buy_obj->creator);
	sprintf(buf,"%s quested",ch->name);
	buy_obj->creator = str_dup(buf);
	...
	sprintf(buf2,"%s quested",ch->name);
...
void quest_request(CHAR_DATA *ch, CHAR_DATA *questmaster, char *argument)
{
	...
	else if(!str_prefix(argument, "hunt"))
		type_of_quest = QUEST_TYPE_HUNT;
	/*
	...
	*/
	...
	if (type_of_quest != QUEST_TYPE_HUNT) {
		...
		if (hunt_quest_timer < 0) {
			sprintf(buf, "The hunt is already on, %s!", ch->name);
			do_say(questmaster, buf);
			return;
		}
		
		if (hunt_quest_timer > 0) {
			sprintf(buf, "You must wait %i hours before calling another hunt.", hunt_quest_timer);
			do_say(questmaster, buf);
			return;
		}
	}
...
void quest_complete(CHAR_DATA *ch, CHAR_DATA *questmaster)
{
	...
	if (completed) {
		...
		if(IS_HARDCORE(ch)) {
			qp_reward = qp_reward * 110/100;
			xp_bonus = xp_bonus * 110/100;
		}
		
		//permanent increase in xp and qp
		qp_reward = qp_reward * 3/2;
		xp_bonus = xp_bonus * 3/2;
		
		if ((double_qp_ticks > 0) || (double_qp_ticks == -1))
			qp_reward *= 2;
...
/* Tsongas 2/27/17
   Quest_Double is a function that allows players to spend gold to double experience or quest points for the entire player base.
   Definitions for tick times and gold cost are located in merc.h
*/

void
quest_double(CHAR_DATA *ch, CHAR_DATA *questman, char *argument) {
	int gold_cost = QUEST_DOUBLE_GOLD_COST;
	int xp_ticks = QUEST_DOUBLE_XP_TICKS;
	int qp_ticks = QUEST_DOUBLE_QP_TICKS;
	char buf[MAX_STRING_LENGTH];
	DESCRIPTOR_DATA *d;
	OBJ_DATA *obj = NULL;
	...
	if (argument[0] == '\0')
	{
		Cprintf(ch, "To double experience or quest points, type 'QUEST DOUBLE <xp|qp>'.\n\r");
		return;
	}

	if(!str_prefix(argument, "xp") && double_xp_ticks == -1)
	{
		Cprintf(ch, "Double experience is currently activated permanently. Come back later when it's not doubled!\n\r");
		return;
	}
	else if(!str_prefix(argument, "xp") && double_xp_ticks > 0)
	{
		Cprintf(ch, "Double experience is currently activated. Come back in %d ticks when it's over!\n\r", double_xp_ticks);
		return;
	}
	else if(!str_prefix(argument, "qp") && double_qp_ticks == -1)
	{
		Cprintf(ch, "Double quest points is currently activated permanently. Come back later when it's not doubled!\n\r");
		return;
	}
	else if(!str_prefix(argument, "qp") && double_qp_ticks > 0)
	{
		Cprintf(ch, "Double quest points is currently activated. Come back in %d ticks when it's over!\n\r", double_qp_ticks);
		return;
	}
	...
	if(ch->gold < gold_cost)
	{
		if(has_bank_note(ch))
		{
			...
			if(obj->value[2] >= (gold_cost * 100))
			{
				sprintf(buf, "I see you don't have enough gold, but that bank note will work just fine!\n\r");
				do_say(questman, buf);
				note_found = TRUE;
			}
		}
		...
		if(note_found == FALSE)
		{
			sprintf(buf, "You need %d gold to double experience.", gold_cost);
			do_say(questman, buf);
			return;
		}
	}

	if(!str_prefix(argument, "xp"))
	{
		do_double(questman, "xp quest");
		...
		{
			obj->value[2] -= (gold_cost * 100);
		}
	
		Cprintf(ch, "You have paid the gods, and they have enabled double experiance points for %d ticks!\n\r", qp_ticks);
		
	}
		
	else if(!str_prefix(argument, "qp"))
	{
		do_double(questman, "qp quest");
		...
		Cprintf(ch, "You have paid the gods, and they have enabled double quest points for %d ticks!\n\r", qp_ticks);
	}
	else
	{
		Cprintf(ch, "To double experience or quest points, type 'QUEST DOUBLE <xp|qp>'.\n\r");
	...
	for (d = descriptor_list; d; d = d->next)
	{
		if (d->connected == CON_PLAYING
			...
			&& d->character != ch)
		{
			Cprintf(d->character, "{R%s {xhas paid the gods for their services. They have have enabled {cdouble %s {xfor {W %d {xticks! {x\n\r",
				ch->name,
				!str_prefix(argument, "xp") ? "experience points" : "quest points",
				!str_prefix(argument, "xp") ? xp_ticks : qp_ticks);
		}
...
do_quest(CHAR_DATA * ch, char *argument)
{
	...
	else if (!str_prefix(arg1, "double"))
	{
		quest_double(ch, questman, arg2);
		return;
	}
	...
	else if(!str_prefix(arg1, "gear20")) {
		gear20(ch);
		return;
	}

	Cprintf(ch, "QUEST commands: POINTS INFO TIME REQUEST COMPLETE LIST BUY DOUBLE GEAR20.\n\r");
	if(TEST_PORT) {
...
void quest_story_hunt(CHAR_DATA *questman, CHAR_DATA *victim) {
	char buf[MAX_STRING_LENGTH];
	int cont = (victim->in_room == NULL ? 0 : victim->in_room->area->continent);
	
	sprintf(buf, "A rare and wondrous beast has been spotted in the realm!");
	do_say(questman, buf);
	sprintf(buf, "It is rumored to carry great treasures.");
	do_say(questman, buf);
	sprintf(buf, "Whoever slays it shall reap the rewards.");
	do_say(questman, buf);
	sprintf(buf, "The hunt will take place on %s.",cont == 0 ? "Terra" : "Dominia");
	do_say(questman, buf);
}
...
//Create the hunt target
void generate_hunt_mob(CHAR_DATA *ch, ROOM_INDEX_DATA *room)
{
	CHAR_DATA *mob = NULL;
	...
	mob = create_mobile(get_mob_index(MOB_VNUM_HUNT_MOB));
	size_mob(ch, mob, 1);
	mob->max_hit = 1;
	mob->hit = mob->max_hit;
	mob->damroll = 1;
	...
	mob->short_descr = str_dup("a rare and wondrous beast");

	if(mob->name != NULL)
		free_string(mob->name);
	mob->name = str_dup("rare wondrous beast");

	if(mob->long_descr != NULL)
		free_string(mob->long_descr);
	mob->long_descr = str_dup("{x[TARGET] A {yr{Yar{ye{x and {mw{Mo{mn{Md{mr{Mo{mu{Ms{x beast stands here.\n\r");
	mob->rot_timer = 25;
	...
	equipment voucher		7
	restring voucher		7
	enchantment voucher	7
	bank note: 5-8K		7
	quest points voucher	7
		double voucher		3
	bank note: 12-15K		3
	skill voucher			3
		recipe voucher		3 
	...
	*/
	if(number_range(0,100) < 25) {
		for(count = 0; min < SET_MIN && count < 100; count++) { //For loop for sanity
			if(number_range(0,200) < 35) { //Partial voucher
				if ((obj_index = get_obj_index(23511)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
				obj_to_char(drop,mob);
				min++;
				max++;
				if(max == SET_MAX)
					break;
			}
			if(number_range(0,200) < 20) { //Bank note, 1-3K
				if ((obj_index = get_obj_index(6521)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
				drop->value[2] = 100000 * number_range(1,3);
				obj_to_char(drop,mob);
				min++;
				max++;
				if(max == SET_MAX)
			...
			/*if(number_range(0,200) < 9) { //Upgrade voucher
				if ((obj_index = get_obj_index(19550)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 9) { //Bounty voucher
				if ((obj_index = get_obj_index(19551)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 9) { //Pardon voucher
				if ((obj_index = get_obj_index(19552)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 9) { //Spellup voucher
				if ((obj_index = get_obj_index(19553)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 7) { //Bank note, 5-8K
				if ((obj_index = get_obj_index(6521)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
				drop->value[2] = 100000 * number_range(5,8);
				obj_to_char(drop,mob);
				min++;
				max++;
				if(max == SET_MAX)
			...
			if(number_range(0,200) < 7) { //Restring voucher
				if ((obj_index = get_obj_index(21018)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 7) { //Enchant voucher
				if ((obj_index = get_obj_index(21019)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 7) { //QP voucher
				if ((obj_index = get_obj_index(21022)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 7) { //EQ voucher
				if ((obj_index = get_obj_index(21017)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 3) { //Bank note, 12-15K
				if ((obj_index = get_obj_index(6521)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
				drop->value[2] = 100000 * number_range(12,15);
				obj_to_char(drop,mob);
				min++;
				max++;
				if(max == SET_MAX)
			...
			if(number_range(0,200) < 3) { //Skill voucher
				if ((obj_index = get_obj_index(10924)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 3) { //Recipe voucher
				if ((obj_index = get_obj_index(19554)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 3) { //Double voucher
				if ((obj_index = get_obj_index(21025)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
			...
			if(number_range(0,200) < 1) { //Loyalty oath
				if ((obj_index = get_obj_index(6488)) == NULL) {
					log_string("Bad vnum for hunt mob drop.");
					continue;
				}
				drop = create_object(obj_index, 58);
				SET_BIT(drop->extra_flags, ITEM_NOLOCATE);
				obj_to_char(drop,mob);
				min++;
				max++;
				if(max == SET_MAX)
...
generate_quest(CHAR_DATA *ch, CHAR_DATA *questman, int quest_type) {
	...
	if(quest_type == QUEST_TYPE_HUNT)
		level = number_range(1, 51);
	else
		level = ch->level;
    ...
    for (victim = char_list; victim != NULL; victim = victim->next) {
		...
		//|| (quest_type != QUEST_TYPE_HUNT && ch->in_room->area->continent != victim->in_room->area->continent)
    ...
    if (quest_type == QUEST_TYPE_HUNT) {
	    ...
	    h2 = -1;
	    h3 = -1;
	    generate_hunt_mob(ch, room);
	    quest_story_hunt(questman, victim);
	...
	for (d = descriptor_list; d; d = d->next)
	{
		if (d->connected == CON_PLAYING
			...
			&& d->character != ch)
		{
			Cprintf(d->character, "{y%s has called a hunt!{x\n\r", ch->name);
		}
	}
	
	    hunt_quest_timer = -1;
	    hunt_continent = victim->in_room == NULL ? 0 : victim->in_room->area->continent;
    }
...
void test_port_promote(CHAR_DATA *ch) {
	...
	/*if ((ch->race == race_lookup("black dragon")) || (ch->race == race_lookup("red dragon")) || (ch->race == race_lookup("green dragon")) || 
	(ch->race == race_lookup("white dragon")) || (ch->race == race_lookup("blue dragon")) || (ch->race == race_lookup("platinum dragon"))  || 
	(ch->race == race_lookup("gold dragon")) || (ch->race == race_lookup("silver dragon")) || (ch->race == race_lookup("copper dragon"))  || 
    (ch->race == race_lookup("iron dragon"))  || (ch->race == race_lookup("hatchling")))*/
	
	if (ch->level < 10) { 
		ch->trust = 10;
		sprintf(buf, "%s 10", ch->name);
	} else { 
		ch->trust = 51;
...
void gear20 (CHAR_DATA *ch) {
	OBJ_DATA *obj;
	Cprintf(ch, "Activating Command: Gear20\n\r");
	obj = create_object(get_obj_index(18330), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(18330), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(17664), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(17664), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(5608), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(10004), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(10004), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(2210), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(2221), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(12011), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(11512), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(9302), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(17636), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(9307), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(10002), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(10424), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(9308), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(2301), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(1395), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(1395), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(11823), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(11823), 0);
	obj_to_char(obj, ch);
	...
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(11088), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(11088), 0);
	obj_to_char(obj, ch);
	
	Cprintf(ch, "Several items may be invis.\n\r");
...
void test_port_gearmelee(CHAR_DATA *ch) {
	...
	obj = create_object(get_obj_index(2980), 0);
        ...
        obj_to_char(obj, ch);
	obj = create_object(get_obj_index(16497), 0);
	obj_to_char(obj, ch);
	obj = create_object(get_obj_index(30818), 0);
	obj_to_char(obj, ch);
