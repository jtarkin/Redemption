 ...
 */
DECLARE_SPEC_FUN(spec_artificer_golem);
...
DECLARE_SPEC_FUN(spec_clan_warleader);
DECLARE_SPEC_FUN(spec_control_soldier);
DECLARE_SPEC_FUN(spec_control_commander);
/* deamon specs */
...
DECLARE_SPEC_FUN(spec_bigot_selina);
DECLARE_SPEC_FUN(spec_bigot_bosco);
...
const struct spec_type spec_table[] =
{
		...
		{"spec_bigot_selina", spec_bigot_selina},
		{"spec_bigot_bosco", spec_bigot_bosco},
		{"spec_artificer_golem", spec_artificer_golem},
		{"spec_clan_warleader", spec_clan_warleader},
		{"spec_control_soldier", spec_control_soldier},
		{"spec_control_commander", spec_control_commander},
		{NULL, NULL}
};
...
spec_doorman(CHAR_DATA * ch)
{
	...
	{
		v_next = victim->next_in_room;
		if (!IS_NPC(victim) && victim->clan == ch->clan)
		{
			act("$n disappears in a flash of light.", victim, NULL, NULL, TO_ROOM, POS_RESTING);
			if (victim->in_room->area->continent == 0)
			{
...
bool spec_bigot_selina(CHAR_DATA *ch)
{
	CHAR_DATA *victim;
	if (!IS_AWAKE(ch) || ch->fighting != NULL)
		return FALSE;

	if (number_range(0,5) != 0) return FALSE;
	...
	for(victim = ch->in_room->people; victim != NULL; victim = victim->next_in_room) 
	{
		...
		if (!can_see(ch, victim) || is_affected(victim, gsn_cloak_of_mind))
			continue;

		if (victim->deity_type == deity_lookup("carnas")
				|| victim->deity_type == deity_lookup("gabriel")
				|| victim->deity_type == deity_lookup("sky")
				|| victim->deity_type == deity_lookup("taelisan")
				|| victim->deity_type == deity_lookup("bhaal"))
		{
			int action = number_range(0,3);

			switch(action)
			{
			case 0:
				do_say(ch, "Prepare to die, heathen worshipper of foul Bosco!");
				do_say(ch, "Follow the light of Selina's children or die!");
				if (IS_SET(ch->off_flags,OFF_BACKSTAB))
					do_backstab(ch,victim->name);
				do_kill(ch, victim->name);
				return TRUE;
			case 1:
				do_say(ch, "Aha! A heathen follower of Bosco!");
				do_say(ch, "Taste my wrath!");
				if (IS_SET(ch->off_flags,OFF_BASH))
					do_bash(ch,victim->name);
				do_kill(ch, victim->name);
				return TRUE;
			case 2:
				do_say(ch, "Selina is the only true goddess!");
				do_say(ch, "Die, follower of Bosco! DIE!");
				do_kill(ch, victim->name);
				return TRUE;
			case 3:
				do_yell(ch, "For Selina's glory! Death to the false worshippers of Bosco's children!");
				do_kill(ch, victim->name);
				return TRUE;
			}
		}			
...
bool spec_bigot_bosco(CHAR_DATA *ch)
{
	CHAR_DATA *victim;
	if (!IS_AWAKE(ch) || ch->fighting != NULL)
		return FALSE;
	...
	for(victim = ch->in_room->people; victim != NULL; victim = victim->next_in_room) 
	{
		...
		if (victim->deity_type == deity_lookup("kallana")
				|| victim->deity_type == deity_lookup("myena"))
		{
			int action = number_range(0,3);
			...
			{
			case 0:
				do_say(ch, "Worshipper of Selina's children, die!");
				do_say(ch, "Bosco's power is supreme!");
				if (IS_SET(ch->off_flags,OFF_BACKSTAB))
					do_backstab(ch,victim->name);
				do_kill(ch, victim->name);
				return TRUE;
			case 1:
				do_say(ch, "Aha! A heathen follower of Selina!");
				do_say(ch, "Taste my wrath!");
				if (IS_SET(ch->off_flags,OFF_BASH))
					do_bash(ch,victim->name);
				do_kill(ch, victim->name);
				return TRUE;
			case 2:
				do_say(ch, "Bosco is the only true god!r");
				do_say(ch, "Die, follower of Selina's children! DIE!");
				do_kill(ch, victim->name);
				return TRUE;
			case 3:
				do_yell(ch, "For Bosco's glory! Death to the false worshippers of Selina's children!");
				do_kill(ch, victim->name);
				return TRUE;
			}
		}	
...
bool spec_artificer_golem(CHAR_DATA *ch)
{
	...
	if (ch->in_room == NULL || ch->pIndexData->vnum != MOB_VNUM_ARTIE_GOLEM)
		return FALSE;

	if (number_percent() > 10) return FALSE;
	...
	if (number_percent() < 30) //based on added parts
	{
		if (number_percent() < 10 && IS_SET(ch->off_flags,OFF_PARRY) && ch->fighting)
		{
			act("$n viciously drives $s hand-drill straight into $N!",ch,NULL,victim,TO_ROOM,POS_RESTING);
			act("$n viciously drives $s hand-drill straight into you!",ch,NULL,victim,TO_VICT,POS_RESTING);
			damage(ch, victim, number_range(ch->level, ch->level * 2), gsn_charge, DAM_PIERCE, TRUE, TYPE_SKILL);
			return TRUE;
		}
		if (number_percent() < 10 && IS_SET(ch->off_flags,OFF_CRUSH) && ch->fighting)
		{
			act("$n smashes $N violently with $s hammer-hand!",ch,NULL,ch->fighting,TO_ROOM,POS_RESTING);
			act("$n violently smashes you with $s hammer-hand!",ch,NULL,ch->fighting,TO_VICT,POS_RESTING);
			damage(ch, ch->fighting, number_range(ch->level, ch->level * 2), gsn_bash, DAM_BASH, TRUE, TYPE_SKILL);
			return TRUE;
		}
		if (number_percent() < 10 && IS_SET(ch->affected_by,AFF_FLYING) && ch->fighting)
		{
			act("With a clatter of gears, $n blasts $N violently with $s mechanical wings!",ch, NULL,ch->fighting, TO_ROOM, POS_RESTING);
			act("With a clatter of gears, $n violently blasts you with $s wings!",ch,NULL,ch->fighting,TO_VICT,POS_RESTING);
			damage(ch, ch->fighting, number_range(ch->level / 2, ch->level), gsn_wingblast, DAM_BASH, TRUE, TYPE_SKILL);
			if (!IS_AFFECTED(ch->fighting,AFF_BLIND) && number_percent() < 30)
			{
				af.where = TO_AFFECTS;
		...
		if (number_percent() < 10 && IS_SET(ch->affected_by,AFF_SANCTUARY))
		{
			if (!ch->fighting && ch->hit < ch->max_hit && !is_affected(ch,gsn_greater_revitalize))
			{
				act("$n's reinforced shell begins to knit itself back together.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				af.where = TO_AFFECTS;
				af.type = gsn_greater_revitalize;
				af.level = ch->level;
				af.duration = 10;
				af.location = APPLY_NONE;
				af.modifier = 3 * ch->level;
				af.bitvector = 0;
				affect_to_char(ch, &af);				return TRUE;
			}
		}
		if (number_percent() < 10 && IS_SET(ch->res_flags,RES_MAGIC))
		{
			if (!ch->fighting)
			{
				act("The runes covering $n's metal frame glow, but nothing happens.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				return TRUE;
			}
			else
			{
				act("The runes on $n flare brilliantly!",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				damage(ch, ch->fighting, number_range(ch->level * 2, ch->level * 3), gsn_fireball, DAM_FIRE, TRUE, TYPE_MAGIC);				return TRUE;
			}
		}
		if (number_percent() < 10 && IS_SET(ch->act, ACT_MAGE))
		{
			if (ch->fighting)
			{
				rand = number_range(0,4);
				if (rand == 0)
					act("{y$n{y intones, 'Enemy detected. Destruction in progress.'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				else if (rand == 1)
					act("{y$n{y intones, 'Initiate protocol: Murder.'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				else if (rand == 2)
					act("{y$n{y intones, 'Crush. Kill. Destroy.'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				else if (rand == 3)
					act("{y$n{y intones, 'More blood for the Machine God!'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				else
					act("{y$n{y intones, 'xy^2 - z^2zyx^3 == TERMINATION'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				return TRUE;
			}
			else
			{
				rand = number_range(0,2);
				if (rand == 0)
					act("{y$n{y intones, 'Security protocol Gamma 4 initiated.'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				else if (rand == 1)
					act("{y$n{y intones, 'Unit 443 could use a new paint job, master.'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				else
					act("{y$n{y intones, 'Unit 443 requests better oil filters.'{x",ch,NULL,NULL,TO_ROOM,POS_RESTING);
				return TRUE;
			}
		}
		return FALSE;
	}
	else // based on gemstones for programming
	{
		for (paf = ch->affected; paf != NULL; paf = paf->next) //count up all the gemstone effects
		{
			if (paf->type == gsn_gemology)
			{
		...
		if (red == 0 && blue == 0 && white == 0 && black == 0 && green == 0) //no gems, abort
			return FALSE;

		//now all the different possibilities
		if (red == 1 && blue == 0 && white == 0 && black == 0 && green == 0 && ch->fighting)
		{
			act("$n emits a small burst of flames from a vent pipe.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_burning_hands(gsn_burning_hands, ch->level, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue == 1 && white == 0 && black == 0 && green == 0 && ch->fighting)
		{
			act("$n emits a small jolt of electricity.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_shocking_grasp(gsn_shocking_grasp, ch->level, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue == 0 && white == 1 && black == 0 && green == 0 && ch->fighting)
		{
			act("$n clanks and clatters and emits a burst of energy.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_cause_critical(gsn_cause_critical, ch->level, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue == 0 && white == 0 && black == 1 && green == 0 && ch->fighting)
		{
			act("$n vomits forth a spray of oil.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_blindness(gsn_blindness, ch->level, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue == 0 && white == 0 && black == 0 && green == 1 && ch->fighting)
		{
			act("$n sprays poisonous lubrication fluid.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_plague(gsn_plague, ch->level, ch, (void*)victim,TARGET_CHAR);
			spell_poison(gsn_poison, ch->level, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red > 1 && blue == 0 && white == 0 && black == 0 && green == 0 && ch->fighting)
		{
			act("$n rattles and fires a spray of scalding hot steam!",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_pyrotechnics(gsn_pyrotechnics, ch->level + red, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue > 1 && white == 0 && black == 0 && green == 0 && ch->fighting)
		{
			act("$n emits a powerful blast of lightning from its core!",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_lightning_bolt(gsn_lightning_bolt, ch->level + blue, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue == 0 && white > 1 && black == 0 && green == 0 && ch->fighting)
		{
			act("$n blasts out with a ray of energy!",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_harm(gsn_harm, ch->level + white, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue == 0 && white == 0 && black > 1 && green == 0 && ch->fighting)
		{
			act("$n emits waves of terrible energy.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_curse(gsn_curse, ch->level + black, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red == 0 && blue == 0 && white == 0 && black == 0 && green > 1 && ch->fighting)
		{
			act("$n emits a wave of anti-magic.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_dispel_magic(gsn_dispel_magic, ch->level + green, ch, (void*)victim,TARGET_CHAR);
		}
		else if (red >= 1 && white >= 1 && black == 0)
		{
			act("A hatch in $n's chest opens and a ray of healing energy floods the room.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
			{
				if (is_same_group(ch,vch) && ch != vch)
				{
					spell_cure_critical(gsn_cure_critical, ch->level + white, ch, (void *) vch, TARGET_CHAR);
					spell_refresh(gsn_refresh, ch->level + white, ch, (void *) vch, TARGET_CHAR);
				}
			}
		}
		else if (green >= 1 && white >= 1 && black == 0)
		{
			act("A small device on $n's chest emits a warm glow.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
			{
				if (is_same_group(ch,vch) && ch != vch)
				{
					spell_refresh(gsn_refresh, ch->level + white, ch, (void *) vch, TARGET_CHAR);
					spell_cure_disease(gsn_cure_disease, ch->level + white, ch, (void *) vch, TARGET_CHAR);
					spell_cure_poison(gsn_cure_poison, ch->level + white, ch, (void *) vch, TARGET_CHAR);
				}
			}
		}
		else if (black >= 1 && white >= 1 && black == 0)
		{
			act("A small device on $n's head emits a warm glow.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
			{
				if (is_same_group(ch,vch) && ch != vch)
				{
					spell_cure_blindness(gsn_cure_blindness, ch->level + white, ch, (void *) vch, TARGET_CHAR);
					spell_cure_serious(gsn_cure_serious, ch->level + white, ch, (void *) vch, TARGET_CHAR);
				}
			}
		}
		else if (blue >= 1 && white >= 1 && black == 0)
		{
			act("$n suddenly radiates with protective energy.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room)
			{
				if (is_same_group(ch,vch) && ch != vch && !is_affected(vch,gsn_harden))
				{
					af.where = TO_AFFECTS;
					af.type = gsn_harden;
					af.level = ch->level;
					...
					af.modifier = (ch->level / -2) - (white * 2) - (blue * 2);
					af.bitvector = 0;
		...
		{
			act("A hatch on $n pops open and a multitude of scuttling clockwork creatures begin repairing it..", ch,NULL, NULL,TO_ROOM, POS_RESTING);
			ch->timer += (green + blue);
			spell_cure_serious(gsn_cure_serious, ch->level + white, ch, (void *) ch, TARGET_CHAR);
		}
		else if (black >= 1 && blue >= 1 && white == 0)
		{
			act("$n carefully rearranges its armor plates.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			if (is_affected(ch,gsn_elemental_protection))
				affect_strip(ch,gsn_elemental_protection);
			spell_elemental_protection(gsn_elemental_protection,ch->level + red,ch,(void *)ch,TARGET_CHAR);
		}
		else if (red == 1 && black == 1 && blue == 1 && white == 0 && green == 1 && ch->fighting)
		{
			act("$n's gemstones glow ominously..",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			act("$n suddenly emits a powerful beam of pure energy!",ch,NULL,NULL,TO_ROOM,POS_RESTING);
			spell_prismatic_spray(gsn_prismatic_spray,ch->level - 3,ch,(void *)victim,TARGET_CHAR);
			if (ch->fighting != NULL)
				spell_channel_energy(gsn_channel_energy,ch->level - 3,ch,(void *)victim,TARGET_CHAR);
		}
		else
		{
			if (number_percent() < 50)
				act("The gemstones embedded in $n fizzle and spark. Nothing happens.",ch,NULL,NULL,TO_ROOM,POS_RESTING);
		}
		return TRUE;
	}
	return FALSE;
...
bool spec_clan_warleader(CHAR_DATA *ch) {
	return;
}

bool spec_control_soldier(CHAR_DATA *ch) {
	return;
}

/*
 * This mob is nasty but a little less nasty than legendaries. He is the boss for the battle to control
 * a zone. I have chosen to model him after Tetragon, but definitely supped up!
 * Do not recover while engaged. (weaker than legendaries)
 * Combat options:
 * Slay charmies
 * Delayed Blast Fireball everyone in the room.
 * Change immunity
 * Summon an animated scimitar
 */
bool spec_control_commander(CHAR_DATA *ch) {
	int immune_bit = 0;
	AFFECT_DATA af;
	...
	if(ch->fighting == NULL) {
		// Recover 100 hp per round while not engaged.
		heal_handler(ch, 100);
		...
		case 1:	
			...
			Cprintf(ch, "You strike a {RFIERCE{x blow!\n\r");
			act("$n draws back $s weapon and strikes a {RFIERCE{x blow!", ch, NULL, NULL, TO_ROOM, POS_RESTING);
			act("$n's ultimate lethal strike deals {RUNSPEAKABLE{x damage to $N!!", ch, NULL, victim, TO_ROOM, POS_RESTING);
			act("$n deals a lethal strike to you!", ch, NULL, victim, TO_VICT, POS_RESTING);
			victim->hit = victim->hit / 10;
			}
			return TRUE;
		case 2:
			act("$n's emits a fiery halo across the entire room!", ch, NULL, NULL, TO_ROOM, POS_RESTING);
			for(victim = ch->in_room->people; victim != NULL; victim = victim->next_in_room) {
				if(victim != ch	&& !is_affected(victim, gsn_delayed_blast_fireball) && !is_safe(ch, victim)) {
					Cprintf(victim, "You are engulfed by a reddish glow!\n\r");
					af.where = TO_AFFECTS;
					af.type = gsn_delayed_blast_fireball;
					af.level = ch->level;
			...
			return TRUE;
		case 3:
			do_emote(ch, "'s aura of invulnerability changes phase!");
			
			switch(dice(1, 8)) {
				case 1:
					immune_bit = IMM_FIRE; break;
				case 2:
					immune_bit = IMM_COLD; break;
				case 3:
					immune_bit = IMM_LIGHTNING; break;
				case 4:
					immune_bit = IMM_ACID; break;
				case 5:
					immune_bit = IMM_POISON; break;
				case 6:
					immune_bit = IMM_SLASH; break;
				case 7:
					immune_bit = IMM_PIERCE; break;
				case 8:
				default:
					immune_bit = IMM_BASH; break;
			}
		...
		case 4:
			...
			do_emote(ch, "'s weapon animates and begings attacking!");
			soldier = create_mobile(get_mob_index(MOB_ANIMATED_SABRE));
			
			if(ch->in_room->area->continent == 1) {
				sprintf(buf,"animated cuervo scythe");
				soldier->name = str_dup(buf);			
				sprintf(buf,"an animated Cuervo's Scythe");
				soldier->short_descr = str_dup(buf);						
				sprintf(buf,"An animated copy of Cuervo's Scythe is here, looking lethal!\n\r");
				soldier->long_descr = str_dup(buf);
			}			
			
			char_to_room(soldier, ch->in_room);
			soldier->hunting = ch->fighting;
			soldier->hunt_timer = 30;
			SET_BIT(soldier->toggles, TOGGLES_NOEXP);		
			return TRUE;
		}
