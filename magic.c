...
void heal_handler(CHAR_DATA *ch, int amount)
{
	if(is_affected(ch,gsn_dissolution))
		amount = amount *0.25;
	ch->hit = UMIN(ch->hit + amount, MAX_HP(ch));
}
...
size_obj(CHAR_DATA * ch, OBJ_DATA * obj, int level)
{
	...
	obj->level = level;
	obj->value[1] = 4;
	obj->value[2] = (level / 4) - 1;
	if (obj->value[2] < 1) obj->value[2] = 1;
}
...
saving_throw(CHAR_DATA *caster, CHAR_DATA *victim, int sn, int level, int diff, int stat, int damtype)
{
    ...
    if (!IS_NPC(victim) 
      && get_skill(victim, gsn_defiance) > 0
      && number_percent() < (get_skill(victim,gsn_defiance) * 4 / 5) + 1
      && damtype == DAM_CHARM)
    {
        Cprintf(victim,"You defy the magic!\n\r");
        act("$n defies the magic!", victim, NULL, NULL, TO_ROOM, POS_RESTING);
        check_improve(victim,gsn_defiance,TRUE,1);
        return;
    }
...
do_cast(CHAR_DATA * ch, char *argument) {
	...
	if(ch->charClass == class_lookup("runist") && power_tattoo_count(ch, TATTOO_LEARN_SPELL))	
	{
		...
		for(spell_lookup = 0; spell_lookup < power_tattoo_count(ch, TATTOO_LEARN_SPELL); spell_lookup++)
		{
			...
			{
				tattoo_obj[spell_lookup] = get_obj_carry_or_wear(ch, spellbook_num);

				if(affect_find(tattoo_obj[spell_lookup]->affected, gsn_paint_power) != NULL)
				{
					pafTattoo[spell_lookup] = affect_find(tattoo_obj[spell_lookup]->affected, gsn_paint_power);
	...
	if (!str_cmp(arg1, "~")) { // ~ prevents shadowcastxxx exploit [by Aql, September 2018]
		is_shadow = TRUE;
	...
	// Tsongas 2/23/17: added bypass for found tattoos with spellbooks
	if ((sn < 1
		|| skill_table[sn].spell_fun == spell_null
		|| (!IS_NPC(ch) && (tmp_lvl < skill_table[sn].skill_level[ch->charClass] || ch->pcdata->learned[sn] == 0))
		|| (!IS_NPC(ch) && (ch->pcdata->any_skill == FALSE && !is_legal_skill(ch, sn))))
		&& tattoo_found == FALSE) {
		Cprintf(ch, "You don't know any spells of that name.\n\r");
		return;
	}

//usion we might want to look here, if we are changing the breath types of the new dragons
	...
	if (sn != gsn_burning_hands 
			...
			&& sn != gsn_chill_touch) //these spells cost normal minimum permanently
	{
	...
	if (!IS_NPC(ch) && number_percent() < get_skill(ch,gsn_freedom)
			&& number_percent() < 20)
	{
		Cprintf(ch, "Your scales grow brightly, conserving your mana.\n\r");
		check_improve(ch,gsn_freedom,TRUE,2);
		mana = 0;
	}
	...
	if (number_percent() > get_skill(ch, sn)) {
		...
		if (!IS_NPC(ch)
			...
			lvl = ch->level;
				...
				|| (ch->charClass == class_lookup("artificer") && ch->race == race_lookup("human")))
		{
			// Mages
			lvl = ch->level;
		}
		else if (ch->charClass == class_lookup("paladin")
		...
		/* hours dependent casting level for elves
           done by Starcrossed modified 2017 by usion */
		...
		if (ch->reclass == reclass_lookup("diviner")
				...
				&& sn != gsn_giant_strength // Added by Aql - diviner penalty only supposed to hit damage spells, not all the invocation group
				&& sn != gsn_stone_skin
				&& sn != gsn_armor
				&& sn != gsn_shield	) {
			modified_lvl = modified_lvl * 9 / 10;
		...
		if (ch->reclass == reclass_lookup("shaman")
				&& (sn == gsn_teleport
						||  sn == gsn_nexus
						||  sn == gsn_portal)) {
			modified_lvl = modified_lvl * 10 / 11;
		}
	...
	if ((skill_table[sn].target == TAR_CHAR_OFFENSIVE
		...
		if (!shadow_cast) {
			damage(ch, victim, 0, sn, DAM_NONE, FALSE, TYPE_SKILL); // Prevents aggressive spells from doubling spelldam [by Aql, September 2018]
		}
	}
	...
	if (shadow_cast == TRUE
		...
		char buf[255];
		sprintf(buf, "~ %s", argument); // ~ prevents shadowcastxxx exploit [by Aql, September 2018]
...
spell_blindness(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (saving_throw(ch, victim, gsn_blindness, modified_level - 4, SAVE_NORMAL, STAT_INT, DAM_ENERGY))
	{
...
spell_blink(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	{
		Cprintf(ch, "Your items halt the gate magic and the spell fizzles.\n\r");
		return;
	}
	...
	while (count < 3 && max_count < 20)
	{
		...
		if ((pexit = in_room->exit[door]) == NULL
				...
				|| (to_room->clan != clan_lookup("noclan")))
		{
			max_count++;
...
spell_carnal_reach(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	{
		Cprintf(ch,"Your magic fizzles as nothing that exists is targeted.\n\r");
		return;
	}
	if (victim == ch 
			|| victim->in_room == NULL)
	{
		Cprintf(ch,"Your summoning magics don't seem to work right on that target.\n\r");
		return;
	}
	/*	
	no_gate flag no means no_transportation spells, shouldn't include summons or carnal
	if (wearing_nogate_item(ch))
	{
		Cprintf(ch, "Your items halt the gate magic and the spell fizzles.\n\r");
		return;
	...
	*/
	...
	{
		Cprintf(ch,"Your summoning magics cannot reach there.\n\r");
		return;
	}	
	if ( victim->level >= modified_level + 13
	...
	{
		Cprintf(ch,"That target is too powerful for you to reach!\n\r");
		return;
	}
	if (victim->fighting != NULL)
	{
		Cprintf(ch,"You cannot reach a target who is in combat!\n\r");
		return;
	}
	if (number_percent() < 20 && ch->no_quit_timer > 0)
	{
		Cprintf(ch, "Your nervousness makes the casting go awry.\n\r");
		return;
	}
	...
	if (!IS_NPC(victim) && ch->pktimer > 0 && victim->no_quit_timer > 0)
	{
		Cprintf(ch, "Give yourself a chance to breathe before reaching someone who's just fought.\n\r");
		return;
	}
...
spell_chain_lightning(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	last_vict = victim;
	modified_level -= 7;					/* decrement damage */
...
spell_cone_of_fear(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	for (vch = ch->in_room->people; vch != NULL; vch = vch->next_in_room) {
		...
		if (IS_NPC(vch) && IS_SET(vch->imm_flags, IMM_SUMMON) && IS_SET(vch->imm_flags, IMM_CHARM))
			continue;
...
spell_confusion(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (saving_throw(ch, victim, sn, modified_level, SAVE_NORMAL, STAT_INT, DAM_MENTAL))
	{
...
spell_continual_light(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (room_is_affected(ch->in_room, gsn_darkness))
	{
		...
		chance = 40 + (modified_level * 3) - (paf->level * 3);
		if (number_percent() > chance)
		{
...
spell_demand(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if ((!str_prefix(arg3,"drop")
			|| !str_prefix(arg3,"give"))
			&& !IS_NPC(victim))
	{
...
spell_denounciation(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (saving_throw(ch, victim, sn, caster_level, SAVE_NORMAL, STAT_INT, DAM_ENERGY))
	{
...
void spell_4xexp( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim       = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    if ( is_affected( victim, sn) )
    {
	Cprintf( victim, "You already learn at the maximum amount.\n\r" );
	return;
    }
    af.where     = TO_AFFECTS;
    af.type      = sn;
    af.duration  = 40; /* Feel free to give more or less */
    af.location  = APPLY_NONE;
    af.modifier  = 4;
    af.bitvector = 0;
    affect_to_char( victim, &af );
    Cprintf( victim, "{YYou begin to learn more quickly from your kills!{x\n\r" );
    act( "{Y$n{x {Wseems to{x {Gshimmer with knowledge.{x\n\r", ch, NULL, NULL, TO_ROOM, POS_RESTING);
    return;
}
...
spell_detonation(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	else
	{
		if (obj_is_affected(obj,gsn_graft)) {
			act("$N is burned by $S weapon as it bursts into flame!", ch, NULL, victim, TO_CHAR, POS_RESTING);
			act("$n detonation causes your your weapon to burst into flames!", ch, NULL, victim, TO_VICT, POS_RESTING);
			...
			act("$n causes $N's weapon to burst into flames!", ch, NULL, victim, TO_ROOM, POS_RESTING);
			damage_effect_pc( ch, obj, TIMER_DETONATE );
//			obj_from_char(obj);
...
spell_displace(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if ((victim = get_char_world(ch, target_name, FALSE)) == NULL
			...
			|| ch->in_room->clan != clan_lookup("noclan")
			|| victim->in_room->clan != clan_lookup("noclan")
	...
	act("$n spirals and twists, and untwists into $N!", ch, NULL, victim, TO_ROOM, POS_RESTING);
		...
		if (vch->in_room == ch->in_room)
		{
			...
			if (vch != ch && !is_safe(ch, vch))
			{
				if (IS_AFFECTED(vch, AFF_FLYING))
					damage(ch, vch, 0, sn, DAM_EARTH, TRUE, TYPE_MAGIC);
				else
					damage(ch, vch, modified_level + dice(2, 8), sn, DAM_EARTH, TRUE, TYPE_MAGIC);
				continue;
			}
		}
...
spell_enchant_weapon(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	{
		Cprintf(ch, "Intelligent weapons cannot be enchanted.\n\r");
		return;
	}
	...
	fail /= 2;
		...
		if ((obj->level >= 51) && !IS_IMMORTAL (ch)){
			Cprintf(ch, "Enchanting a weapon past level 51 is no longer possible without an Immortal.\n\r");
			return;
		}
...
spell_enlargement(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	heal_handler(victim, modified_level * 4);
...
spell_fear(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	paf = affect_find(ch->affected, gsn_fear);
	if (paf != NULL)
	{
		if (paf->modifier <= -10)
		{
			Cprintf(ch,"They are already afraid enough.\n\r");
			return;
		}
	}
	...
	saved = saving_throw(ch, victim, sn, caster_level, SAVE_HARD, STAT_INT, DAM_MENTAL);
...
spell_fly(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (is_affected(victim, gsn_earth_breath))
	{
		act("This person is grounded by earth and cannot fly!", victim, NULL, ch, TO_CHAR, POS_RESTING);
		Cprintf(ch, "You are grounded by the earth at the moment.\n\r");
		return;
	}
	af.where = TO_AFFECTS;
	af.type = sn;
...
spell_gate(int sn, int level, CHAR_DATA * ch, void *vo, int target) {
	...
	if ((victim = get_char_world_finished_areas(ch, target_name, FALSE)) == NULL
			...
			|| victim->in_room == NULL)
	{
		Cprintf(ch,"Your gate magics failed since there was no target.\n\r");
		return;
	}
	if (victim->in_room->clan != clan_lookup("noclan")
			...
			|| IS_SET(ch->in_room->room_flags, ROOM_ARENA)     )
	{
		Cprintf(ch,"Your gate magic could not reach that target's location.\n\r");
		return;
	}
	if (victim->level > modified_level + 3
			|| (!IS_NPC(victim) && victim->level >= LEVEL_IMMORTAL)
			|| (IS_NPC(victim) && IS_SET(victim->imm_flags, IMM_SUMMON))
			|| (IS_NPC(victim) && IS_AFFECTED(victim, AFF_CHARM)))
	{
		Cprintf(ch,"That target was too powerful for you to gate to!\n\r");
		return;
	}
	if (number_percent() < 15 && ch->no_quit_timer > 0 )
	{
		Cprintf(ch, "Your nervousness makes the casting go awry.\n\r");
		return;
	}
	...
	if (ch->clan->loner && !IS_NPC(victim))
	{
		Cprintf(ch, "Those without a clan cannot gate to players.\n\r");
		return;
	}
	if (IS_NPC(victim) && victim->pIndexData->pShop != NULL)
	{
		Cprintf(ch,"You cannot gate to shopkeepers.\n\r");
		return;
	}
	if (!IS_NPC(victim) && ch->clan != victim->clan && victim->clan != clan_lookup("noclan"))
	{
		Cprintf(ch, "You cannot gate to an enemy clanmember.\n\r");
		return;
	}
	if (victim->in_room->area->continent != ch->in_room->area->continent) {
		Cprintf(ch, "Inter-continental magic is forbidden.  You failed.\n\r");
...
spell_haste(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	{
		Cprintf(ch, "That target is moving quite fast enough, thank you.\n\r");
		return;
	}
	if (is_affected(victim, sn))
	{
...
spell_heal(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	heal_handler(victim, heal_amount);
...
spell_heat_metal(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (!saving_throw(ch, victim, sn, caster_level + 2, SAVE_NORMAL, STAT_CON, DAM_FIRE)
...
void spell_holy_word(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	CHAR_DATA *vch_next;
	int dam;
	int caster_level, modified_level, bonus_level;
	...
	for (vch = ch->in_room->people; vch != NULL; vch = vch_next)
	{
		...
		if (is_same_group(ch, vch)
				|| (!IS_NPC(vch) && vch->clan == ch->clan && is_clan(ch))
				|| (!IS_NPC(vch) && vch->level < ch->level - 8))
		{
			...
			if ((IS_GOOD(ch) && IS_GOOD(vch)) ||
				...
				bonus_level = 2;
			else if ((IS_GOOD(ch) && !IS_GOOD(vch)) ||
					(IS_NEUTRAL(ch) && !IS_NEUTRAL(vch)) ||
					(IS_EVIL(ch) && !IS_EVIL(vch)))
				bonus_level = -4;
			spell_frenzy(gsn_frenzy, level + bonus_level, ch, (void *) vch, TARGET_CHAR);
			spell_bless(gsn_bless, level + bonus_level, ch, (void *) vch, TARGET_CHAR);
			spell_heal(gsn_heal, level + bonus_level, ch, (void *) vch, TARGET_CHAR);
		}
		else
		{
			if ((IS_GOOD(ch) && IS_GOOD(vch)) ||
			...
			if (!is_safe(ch, vch))
			{
				spell_curse(gsn_curse, level + bonus_level, ch, (void *) vch, TARGET_CHAR);
				spell_heat_metal(gsn_heat_metal, level + bonus_level, ch, (void *) vch, TARGET_CHAR);
				Cprintf(vch, "You are struck down!\n\r");
				dam = spell_damage(ch, vch, modified_level + bonus_level, SPELL_DAMAGE_MEDIUM, TRUE);
...
spell_lightning_spear(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (is_safe(ch,victim) || (IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim))
	{
...
spell_hypnosis(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (saving_throw(ch, victim, sn, caster_level, SAVE_HARD, STAT_WIS, DAM_CHARM))
	{
		Cprintf(ch, "Your victim resists your hypnotic suggestion!\n\r");
		act("You resist $n's hypnotic suggestion!\n\r",ch, NULL, victim, TO_VICT, POS_SLEEPING);
		return;
	}
...
spell_karma(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	{
                if (victim == ch)
                        Cprintf(ch, "Your good karma is refreshed.\n\r");
                else
                {
                        act("$N refreshed your good karma.", victim, NULL, ch, TO_CHAR, POS_RESTING);
                        Cprintf(ch, "You refresh the good karma.\n\r");
                }
                affect_refresh(victim, sn, modified_level / 3);
                return;
	}
    ...
    act("$n will now have good karma.", victim, NULL, NULL, TO_ROOM, POS_RESTING);
    Cprintf(victim, "You feel the influence of good karma.\n\r");
...
for (vch = char_list; vch != NULL; vch = vch_next)
	{
		...
		if (vch->in_room == ch->in_room)
		{
			...
			if (vch != ch && !is_safe(ch, vch))
			{
				act ("Waves of acid rain called by $n shower down upon $N.", ch, NULL, victim, TO_ROOM, POS_RESTING);
				act ("Waves of acid rain shower down upon $N!", ch, NULL, victim, TO_CHAR, POS_RESTING);
				act("Acid rain comes down from $n's hand and showers upon you!", ch, NULL, victim, TO_VICT, POS_RESTING);
				damage(ch, vch, modified_level + dice(2, 8), sn, DAM_ACID, TRUE, TYPE_MAGIC);
				//maybe we should do an acid room effect similar to cloudkill here? 
				continue;
			}
		}
	}
			...
			if(!IS_OBJ_STAT(obj_aff, ITEM_SOUND_PROOF))
			{
				...
				else
				{
					if (number_percent() <= 35)
					{
						...
						case ITEM_FOOD:
						case ITEM_PILL:
							if (obj_aff->carried_by != NULL)
								act("$p is destroyed by the powerful shriek.", obj_aff->carried_by, obj_aff, NULL, TO_ALL, POS_RESTING);
							else if (obj_aff->in_room != NULL && obj_aff->in_room->people != NULL)
								act("$p is destroyed by the powerful shriek.", obj_aff->in_room->people, obj_aff, NULL, TO_ALL, POS_RESTING);
							extract_obj(obj_aff);
							break;
						default:
							if ( damage_effect_pc( ch, obj_aff, TIMER_SHRIEK ) ) // -K
							{
								if (obj_aff->carried_by != NULL)
									act("$p is temporarily damaged by the powerful shriek.", obj_aff->carried_by, obj_aff, NULL, TO_ALL, POS_RESTING);
								else if (obj_aff->in_room != NULL && obj_aff->in_room->people != NULL)
									act("$p is temporarily damaged by the powerful shriek.", obj_aff->in_room->people, obj_aff, NULL, TO_ALL, POS_RESTING);
							}
							break;
						}
					}
...
spell_slow(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	{		
		...
		Cprintf(victim, "You feel your supernatural speed ebbing away.\n\r");
		if (victim != ch)
			act("$n begins to move at a more normal speed.", victim, NULL, NULL, TO_ROOM, POS_RESTING);
		return;
	}
...
spell_spirit_link(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if ((victim = get_char_world(ch, target_name, FALSE)) == NULL 
			...
			|| victim->in_room == NULL)
	{
		Cprintf(ch,"Your teleportation magic fizzles without an existing target.\n\r");
		return;
	}
	...
	if (!IS_NPC(victim) && ch->clan != victim->clan && victim->clan != clan_lookup("noclan"))
	{
		Cprintf(ch, "You cannot spirit link to an enemy clanmember.\n\r");
		return;
	}
	...
	if (!can_see_room(ch, victim->in_room)
			...
			|| victim->in_room->clan != clan_lookup("noclan"))
	{
		Cprintf(ch,"Your spirit magic cannot reach that location.\n\r");
		return;
	}
	if (victim->level >= modified_level + 13
			|| (IS_NPC(victim) && IS_AFFECTED(victim, AFF_CHARM)
	...
	{
		Cprintf(ch, "That target was too powerful for you to spirit link to!\n\r");
		return;
	}
	if (number_percent() < 10 && ch->no_quit_timer > 0)
	{
		Cprintf(ch, "Your nervousness makes the casting go awry.\n\r");
		return;
	}
...
spell_summon(int sn, int level, CHAR_DATA * ch, void *vo, int target) {
	...
	/*
	no_gate items are now turning into no_transport and shouldn't affect summon
	if (wearing_nogate_item(ch))
	{
		Cprintf(ch, "One of your items is halting your summoning magic.\n\r");
		return;
	}
	*/
	...
	if ((victim = get_char_world_finished_areas(ch, target_name, FALSE)) == NULL
			...
			|| victim->in_room == NULL)
	{
		Cprintf(ch,"Your summon magics failed since there was no target.\n\r");
		return;
	}
	if (ch->in_room->clan != clan_lookup("noclan")
			...
			|| IS_SET(ch->in_room->room_flags, ROOM_ARENA)    )
	{
		Cprintf(ch,"Your summoning magic could not reach that target's location.\n\r");
		return;
	}
	if (victim->level > modified_level + 3
			|| (!IS_NPC(victim) && victim->level >= LEVEL_IMMORTAL)
			...
			|| (IS_NPC(victim) && IS_AFFECTED(victim, AFF_CHARM)))
	{
		Cprintf(ch,"That target was too powerful for you to summon!\n\r");
		return;
	}
	if (number_percent() < 20 && ch->no_quit_timer > 0 && !IS_NPC(victim))
	{
		Cprintf(ch, "Your nervousness makes the casting go awry.\n\r");
		return;
	}
	if (number_percent() < 20 && victim->no_quit_timer > 0 && !IS_NPC(victim))
	{
		Cprintf(ch, "Your summoning magics fizzle due to your target's nervousness.\n\r");
		return;
	}
	...
	if (!IS_NPC(victim) && ch->pktimer > 0 && victim->no_quit_timer > 0)
	{
		Cprintf(ch, "Give yourself a chance to breathe before summoning someone who's just fought.\n\r");
		return;
	}
	if (!IS_NPC(victim) && IS_SET(victim->act, PLR_NOSUMMON))
	{
...
spell_taunt(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	{
		Cprintf(ch, "Your opponent manages to resist the demon's vile taunting!\n\r");
		Cprintf(victim, "You manage to ignore the demon's vile taunting!\n\r");
		return;
	}
...
spell_teleport(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (wearing_nogate_item(ch) && victim == ch)
	{
		Cprintf(ch, "Your items halt the gate magic and the spell fizzles.\n\r");
		return;
	}
...
spell_wrath(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	for (tClan = clan_list; tClan != NULL; tClan = tClan->next)
	{
		if (!str_prefix(buf,tClan->name))
		{
			found = TRUE;
			break;
		}
	...
	if (!found || !tClan->clanner || tClan->loner)
	{
...
spell_weaponsmith(int sn, int level, CHAR_DATA * ch, void *vo, int target)
{
	...
	if (!str_prefix(arg3,"sword"))
		i = 1;
	else if (!str_prefix(arg3,"dagger"))
		i = 2;
	else if (!str_prefix(arg3,"spear"))
		i = 3;
	else if (!str_prefix(arg3,"mace"))
		i = 4;
	else if (!str_prefix(arg3,"axe"))
		i = 5;
	else if (!str_prefix(arg3,"whip"))
		i = 6;
	else if (!str_prefix(arg3,"flail"))
		i = 7;
	else if (!str_prefix(arg3,"polearm"))
		i = 8;
	else if (!str_prefix(arg3,"dagger"))
		i = 9;		
	else
	{
		...
		else
		{
			...
			wskills[1] = victim->pcdata->learned[gsn_sword];
			wskills[2] = victim->pcdata->learned[gsn_dagger];
			wskills[3] = victim->pcdata->learned[gsn_spear];
			wskills[4] = victim->pcdata->learned[gsn_mace];
			wskills[5] = victim->pcdata->learned[gsn_axe];
			wskills[6] = victim->pcdata->learned[gsn_flail];
			wskills[7] = victim->pcdata->learned[gsn_whip];
			wskills[8] = victim->pcdata->learned[gsn_polearm];
		...
		//if they have no skills, give them knuckles
		if (wskills[i] == 0) {
			i = 0;
		}
	}
	...
	case 5:
		...
		break;
	case 6:
		obj = create_object(get_obj_index(OBJ_VNUM_MAGICAL_WHIP), victim->level);
		break;
	case 7:
		obj = create_object(get_obj_index(OBJ_VNUM_MAGICAL_FLAIL), victim->level);
		break;
	case 8:
		obj = create_object(get_obj_index(OBJ_VNUM_MAGICAL_POLEARM), victim->level);
		break;
	default:
...
spell_fire_breath(int sn, int level, CHAR_DATA * ch, void *vo, int target) {
	...
	for (vch = victim->in_room->people; vch != NULL; vch = vch_next) {
		...
		damage(ch, vch, dam, sn, DAM_FIRE, TRUE, TYPE_MAGIC);
		if (vch != NULL && vch->in_room == ch->in_room)
...
//usion the new breath effects for elements need to be coded here
