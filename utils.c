...
/*
 * Returns a fully uppercase string.
 */
char *
cap_str(const char *str) {
    static char strcap[MAX_STRING_LENGTH];
    int i;
    for (i = 0; str[i] != '\0'; i++) {
...
/*Encrypts an entire password*/
char * long_crypt(char *pwd, char *salt) {
	static char pwdcrypt[MAX_STRING_LENGTH]; /*The crypted password*/
	int pwdlen; /*The length of the password for efficiency so it doesn't have to check every loop*/
	char *pwdpos; /*Used in loop to get only the uncrypted part of the password*/
	/*Empty ecrypt*/
	pwdcrypt[0] = '\0';
	/*Ensure password is not overly long*/
	pwdlen = strlen(pwd);
	if (pwdlen > 128) {
		sprintf(pwdcrypt, "~");
		return pwdcrypt;
	}
	/*Take the letters 8 at a time and add their crypt to the end of crypt*/
	for(pwdpos = pwd; pwdpos - pwd < pwdlen; pwdpos += 8) {
		sprintf(pwdcrypt, "%s%s", pwdcrypt, crypt(pwdpos, salt));
	}
	return pwdcrypt;
}

/*Returns 1 if the password matches the character's password, and 0 if it doesn't.*/
int is_pass(CHAR_DATA *ch, char *pwd) {
	return (strcmp(long_crypt(pwd, ch->pcdata->pwd), ch->pcdata->pwd))?0:1;
}
