...
typedef struct class_progress_data CLASS_PROGRESS_DATA;
...
#define MAX_SLIVER      9
#define MAX_HK_CLAIMS   6
#define MAX_DAMAGE_MESSAGE   80
#define MAX_LEVEL       60
#define LEVEL_HERO         (MAX_LEVEL - 9)
#define LEVEL_IMMORTAL        (54)
#define AMMO_WEIGHT      5
...
#define OLD_NOTE_EDITOR 0
#define NOTE_STRING_EDITOR 1
...
#define PUNISH_LAG 1
#define PUNISH_NOCHAN 2
#define PUNISH_NOTELL 3
#define PUNISH_NONOTE 4
#define PUNISH_NOTITLE 5
#define MAX_PUNISH 6
...
#define MOB_ANIMATED_SABRE 3772
#define MOB_AREA_COMMANDER 3371

#define MOB_VNUM_ARTIE_GOLEM 		4
#define MOB_VNUM_HUNT_MOB		31185
...
#define OBJ_VNUM_HUNTER_HAWK	   10905
...
#define OBJ_VNUM_CREATE_RING	   8
#define OBJ_VNUM_CREATE_STAFF	   9
#define OBJ_VNUM_CREATE_WAND	   10
...
#define DAM_WIND				22
#define DAM_EARTH				23
#define DAM_DIVINE				24
#define DAM_SHADOW				25
...
#define IMM_WIND				(cc)
#define IMM_EARTH				(dd)
#define IMM_DIVINE				(ee)
#define IMM_SHADOW				(ff)
...
#define RES_WIND				(cc)	
#define RES_EARTH				(dd)
#define RES_DIVINE				(ee)
#define RES_SHADOW				(ff)
...
#define VULN_RAIN               (aa)
#define VULN_WIND				(bb)
#define VULN_EARTH				(cc)
#define VULN_DIVINE				(dd)
#define VULN_SHADOW				(ee)
...
#define OBJ_VNUM_FLETCH_AMUNITION  	28
#define OBJ_VNUM_CLAN_WARBANNER		8321
...
#define WEAPON_WIND		   (Z)
#define WEAPON_EARTH	   (aa)
#define WEAPON_DIVINE	   (bb)
#define WEAPON_SHADOW	   (cc)
...
#define ROOM_VNUM_DRAGON_CHANGE       23579
#define	ROOM_VNUM_CHALLENGE	       1200 // TEMPORARY ROOM VNUM

/* rooms used for CTF */
#define ROOM_CTF_JAIL(team) ((team) == CTF_BLUE ? 2646 : 2613)
#define ROOM_CTF_JAILER(team) ((team) == CTF_BLUE ? 2645 : 2612)
#define ROOM_CTF_FLAG(team) ((team) == CTF_BLUE ? 2651 : 2601)
#define ROOM_CTF_PREP(team) ((team) == CTF_BLUE ? 2666 : 2665)

#define OBJ_CTF_FLAG(team) ((team) == CTF_BLUE ? 2620 : 2619)
#define CTF_SCORE(team) ((team) == CTF_BLUE ? ctf_score_blue : ctf_score_red )
#define CTF_OTHER_TEAM(team) ((team) == CTF_BLUE ? CTF_RED : CTF_BLUE )
#define MOB_CTF_JAILER(team) ((team) == CTF_BLUE ? 2604 : 2603 )
 ...
 */
#define ROOM_DARK    (A)
#define	ROOM_CHALLENGE	(B)
#define ROOM_NO_MOB     (C)
#define ROOM_INDOORS    (D)
#define ROOM_ARENA      (E)
#define ROOM_FERRY      (F)
#define	ROOM_WORKSHOP	(G)
...
#define DIR_UP             4
#define DIR_DOWN           5
#define	MAX_DIR		6 // a useful sanity trap
...
struct char_data
{
	...
	char *rptitle;
	int group;
	CLAN_DATA *clan;  //new clan pointer
	//use ch->clan->stuff!
	CLAN_DATA *spy_clan; //teldin-> for spying, no longer keep track of clans via number
	CLAN_DATA *join_clan;
	int outcast_timer;
	int sex;
	int charClass;
	...
	bool toggle_dchange;
	int	shop_mob_vnum; //shop mob vnum, set on players
	bool is_player_shop; //to check when saving
	char *shop_discount[2]; //shop clan discounts, set on mob
	int	shop_discount_val[2]; //shop discount amounts, set on mob 50-200
};
...
#define QUEST_TYPE_HUNT (6)

#define QUEST_DOUBLE_XP_TICKS 100
#define QUEST_DOUBLE_QP_TICKS 100
#define QUEST_DOUBLE_GOLD_COST 10000

#define REMORT_DOUBLE_XP_TICKS 100
...
/*
 * Data about an inactive character class.
 */
struct class_progress_data
{
   char *class_name; // The name of the class in the class table.
   int level;
   int reclass;
   int exp_total;
   int cp;
...
/*
 * challenge data for ease of tracking what belongs to it
 */
typedef struct challenge_data
{
    CHAR_DATA* target;
    CHAR_DATA* opponent;
    int issued;
...
struct pc_data
{
	...
	int artie_learned; //how many shaper "learn" groups you have left
	int group1_learned; //to keep track of which groups you add
	int group2_learned;

	int punish_hoursleft[6];
	int punish_daysleft[6];
	int punish_levelsleft[6];
	int last_hour; //to keep track of hoursleft

	int claims_remaining;
	int shaper_penalty;

	int note_editor;

	CLASS_PROGRESS_DATA class_progression[MAX_CLASS];
};
...
struct obj_data
{
	...
	int special[5];
	int extra[5];
    int iw_level[7]; // hit, dam, weapon flag, stat, mod, crafted, mod
	int clan_status;
	char *respawn_owner;
	char *creator;
	long shop_price_gold; //special values for shops
	int shop_price_silver; //special values for shops
};
...
{
	RESET_DATA *next;
	char command;     /*Type O=obj, M=mob, etc...*/
	int arg1;         /*Resetting Item/Mob Vnum*/
	int arg2;         /*Min*/
	int arg3;         /*Wear Location/containing object/room vnum*/
	int arg4;         /*Max*/
	int percent;      /*percent*/
};
...
struct area_data
{
	...
	bool gateable; 	//transport magics allowed? works as no_gate for every room
	bool autocontrol;
...
extern int gsn_elemental_protection;
...
extern int gsn_hatred;
extern int gsn_promise;
...
extern int gsn_pindown;
extern int gsn_bleeder;
extern int gsn_stun;
extern int gsn_purify;
extern int gsn_hallow;
extern int gsn_fletch;
extern int gsn_hideout;

//usion the defines for the new breath types are here
extern int gsn_divine_breath;
extern int gsn_shadow_breath;
extern int gsn_water_breath;
extern int gsn_earth_breath;
extern int gsn_wind_breath;

extern int gsn_blight;
extern int gsn_shadow_bolt;
extern int gsn_smite;

extern int gsn_brilliance;
extern int gsn_dominate;
extern int gsn_aura_of_peace;
extern int gsn_hibernate;
extern int gsn_harden;

extern int gsn_nova;
extern int gsn_defiance;
extern int gsn_dullness;
extern int gsn_freedom;
extern int gsn_preserve;
extern int gsn_mindleech;
extern int gsn_imbue;
extern int gsn_wingblast;
extern int gsn_absorption;
extern int gsn_quicksilver;
extern int gsn_reduce;
extern int gsn_burrow;
extern int gsn_battleroar;
extern int gsn_mystic_stone;
extern int gsn_shinai;
extern int gsn_flameguard;
extern int gsn_edge;
extern int gsn_iron_guard;
extern int gsn_razorscales;

extern int gsn_smokebomb;

extern int gsn_makegolem;
extern int gsn_destroy_magic;
extern int gsn_disassemble;
extern int gsn_infuse;
extern int gsn_repair_golem;
extern int gsn_wand_mastery;
extern int gsn_craft_mastery;

extern int gsn_create_ring;
extern int gsn_create_wand;
extern int gsn_create_staff;
extern int gsn_flux;
extern int gsn_fuse;
extern int gsn_graft;
extern int gsn_incinerate;
extern int gsn_create_forge;
extern int gsn_forge;
extern int gsn_golem_mastery;
extern int gsn_toolbelt;
...
#define IS_HARDCORE(ch)		(IS_SET((ch)->wiznet, HARDCORE))
...
#define LAST_COMMAND    "../area/last_command.txt"  /*For the signal handler.*/
#endif
...
#define COUNT_FILE		"../area/count.txt"
#define CLAN_FILE		"../area/clanleaders.txt"
#define CLAN_DATA_FILE	"../area/clandata.txt" //new - teldin
#define OBJ_SAVE_FILE 	"../area/persistent.txt"//persistent
#define SHOPS_SAVE_FILE 	"../area/playershops.txt"
#define TIME_FILE		"../area/timefile.txt"
...
//usion, the new elemental effects need to be defined here
...
int get_dragon_breaths( CHAR_DATA* ch ); // header for general use -K
...
//macros turned into functions - Teldin
...
CD *get_pc_world(CHAR_DATA * ch, char *argument, int allowIntercontinental);
...
void load_persistent_containers(void);
void save_persistent_containers(void);
...
void offline_learning(CHAR_DATA * ch);
...
#define MAX_REMORT_RACE 42
#define MAX_REMORT_CLASS 8
#define MAX_REMORT_SUBCLASS 4
#define MAX_RECLASS 34
