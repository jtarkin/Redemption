...
format_obj_to_char(OBJ_DATA * obj, CHAR_DATA * ch, bool fShort)
{
	...
	if (obj_is_affected(obj, gsn_graft))
		strcat(buf, "({gGrafted{x) ");
		
	if (obj_is_affected(obj, gsn_toolbelt))
			strcat(buf, "({mToolbelt{x) ");
	...
	if (affect_find(obj->affected, gsn_soul_rune))
		strcat(buf, "(Ghostly) ");
	if (affect_find(obj->affected, gsn_imbue))
		strcat(buf, "({cImbued{x) ");
	if (affect_find(obj->affected, gsn_infuse))
		strcat(buf, "({yInfused{x) ");	
	if (affect_find(obj->affected, gsn_flameguard))
		strcat(buf, "({rFlameguarded{x) ");
	if (affect_find(obj->affected, gsn_damaged) || affect_find(obj->affected_v, gsn_damaged))
		strcat(buf, "({yDamaged{x) ");
...
show_list_to_char(OBJ_DATA * list, CHAR_DATA * ch, bool fShort, bool fShowNothing)
{
	...
	for (obj = list; obj != NULL; obj = obj->next_content) {
		...
		if ( count > 1000 ) { // Prevents item overflow crash [by Aql, September 2018]
			Cprintf(ch, "There are way too many items here.\n\r");
			bug("%s tried to view too many items.", ch->name);
			return;
		}
	}
...
do_autolist(CHAR_DATA * ch, char *argument)
{
	...
	Cprintf(ch, "autofollow        %s\n\r", (IS_SET(ch->act, PLR_AUTOFOLLOW)) ? "ON" : "OFF");
	...
	if (ch->pcdata->note_editor == 1) {
		Cprintf(ch, "You are using the new note editor.\n\r");
	} else {
		Cprintf(ch, "You are using the classic note editor.\n\r");
	}
	...
	if (IS_SET(ch->act, PLR_NOHINTS))
		Cprintf(ch, "You do not see hunt quest hints.\n\r");
	else
		Cprintf(ch, "You will see hunt quest hints.\n\r");
}
...
void
do_autofollow(CHAR_DATA * ch, char *argument)
{
	if(IS_NPC(ch))
		return;
	...
	{
		Cprintf(ch, "Group autofollow removed.\n\r");
		REMOVE_BIT(ch->act, PLR_AUTOFOLLOW);
	}
	else
	{
		Cprintf(ch, "You will now automatically follow your group leader.\n\r");
...
void
do_nohints(CHAR_DATA * ch, char *argument)
{
	if(IS_NPC(ch))
		return;
	...
	if (IS_SET(ch->act, PLR_NOHINTS))
	{
		Cprintf(ch, "You will now see hunt quest hints.\n\r");
		REMOVE_BIT(ch->act, PLR_NOHINTS);
	}
	else
	{
		Cprintf(ch, "You will no longer see hunt quest hints.\n\r");
		SET_BIT(ch->act, PLR_NOHINTS);
	}
}
...
do_prompt(CHAR_DATA * ch, char *argument)
{
	...
	if (!strcmp(argument, "all"))
		...
		strcpy(buf, "{x<%r><%e>%c<%hhp %mm %vmv %ddp %Xtnl>%c<Enemy:%l>{x");
	else
	{
...
do_look(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || !str_cmp(arg1, "auto"))
	{
		...
		if (room_is_affected(ch->in_room, gsn_create_forge))
			Cprintf(ch, "{yA crafter's workshop has been set up in the room.{x\n\r");
		...
		if (room_is_affected(ch->in_room, gsn_aura_of_peace))
			Cprintf(ch, "The room feels sleepy and peaceful to be in.\n\r");
		...
		if (room_is_affected(ch->in_room, gsn_hideout)) {
			Cprintf(ch, "You see evidence that someone has been living here.\n\r");
		}
...
do_peer(CHAR_DATA * ch, char *argument)
{
	...
	if (room_is_affected(pexit->u1.to_room, gsn_web))
		...
		if (room_is_affected(pexit->u1.to_room, gsn_aura_of_peace))
		{
			Cprintf(ch, "The room feels sleepy and peaceful to be in.\n\r");
		}		
...
// END
	...
	else
	{
		...
		if (victim->hunting)
		{
			...
			{
				sprintf(buf, "%s",victim->hunting->short_descr);
			}
			else
			{
				sprintf(buf, "%s",victim->hunting->name);
			}
		}
		else
		{
	...
	if (!IS_NPC(victim) && IS_HARDCORE(victim))
		Cprintf(ch, "{c%-*s{x %-*d\n\r", name_col, "Claims Remaining:", col, victim->pcdata->claims_remaining);
	else
		Cprintf(ch, "\n\r");
...
//check here for dragon breath
	if (is_dragon(victim))
	{
		sprintf(buf, "%d of %d",
				victim->breath / 10,
				get_dragon_breaths( victim ) / 10);
...
show_char_stats_to_char(CHAR_DATA * victim, CHAR_DATA * ch)
{
	...
	if (IS_IMMORTAL(victim) && !IS_NPC(victim))
	{
		...
		Cprintf( ch, "{c%-*s{x %-*d  \n\r",
		  name_col, "Security:", col, victim->pcdata->security );
	}
...
do_new_score(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch) && IS_HARDCORE(ch))
		Cprintf(ch, "You are {rRuthless{x.\n\r");
...
do_old_score(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_HARDCORE(ch))
		Cprintf(ch, "You are {rRuthless{x.  You have %d claims left.\n\r", ch->pcdata->claims_remaining);
	...
	if ( is_dragon( ch ) )
		Cprintf(ch, ", {G%d{x/{R%d{x breaths", ch->breath / 10, get_dragon_breaths( ch ) / 10 );
    ...
    if (victim->affected != NULL || IS_AFFECTED(victim, AFF_HIDE)) {
        ...
        for (paf = victim->affected; paf != NULL; paf = paf->next) {
            ...
            if (ch->level >= 20) {
                ...
                } else if (paf->type == gsn_berserk && paf->location == APPLY_NONE) {
                    Cprintf(ch, ": gradually recover %d more hp", paf->modifier);	
				} else if(paf->type == gsn_bleeder) {
					Cprintf(ch, ": gradually bleed out hp, leaving blood pools, for %d hours", paf->duration);
				} else if(paf->type == gsn_chi_ei) {
					Cprintf(ch, ": modifies counter attack by %d", paf->modifier);
                } else if (paf->type == gsn_toolbelt) {
					if (paf->modifier == 0)
						Cprintf(ch, ": bronze gear gadget lasts for %d hours", paf->duration);
					else if (paf->modifier == 1)
						Cprintf(ch, ": brass cogwheel gadget lasts for %d hours", paf->duration);
					else if (paf->modifier == 2)
						Cprintf(ch, ": grappling hook gadget lasts for %d hours", paf->duration);
					else if (paf->modifier == 3)
						Cprintf(ch, ": chain-link net gadget lasts for %d hours", paf->duration);
					else if (paf->modifier == 4)
						Cprintf(ch, ": triple multitool gadget lasts for %d hours", paf->duration);
					else if (paf->modifier == 5)
						Cprintf(ch, ": ether goggles gadget lasts for %d hours", paf->duration);
					else if (paf->modifier == 6)
						Cprintf(ch, ": glass hummingbird gadget lasts for %d hours", paf->duration);
					else
						Cprintf(ch, ": clockwork gyrocopter gadget lasts for %d hours", paf->duration);
                }else if (paf->type == gsn_hideout) {
					if (paf->modifier == 0) {
						Cprintf(ch, ": you are searching for a new home");
					} else {
						room = get_room_index(paf->modifier);
						Cprintf(ch, ": you have built a hideout in '%s' that lasts %d hours", room->name, paf->duration);
					}
				} else {
...
do_time(CHAR_DATA * ch, char *argument)
{
	...
	extern int double_xp_ticks, double_qp_ticks;
	char *suf;
	int day;
	...
	Cprintf(ch,"It is {c%d{x o'clock %s.\n\r",(time_info.hour % 12 == 0) ? 12 : time_info.hour % 12, time_info.hour >= 12 ? "pm" : "am");
	Cprintf(ch,"It is the day of %s, on the %d%s of the month of %s.\n\r",day_name[day % 7],day, suf, month_name[time_info.month]);
	Cprintf(ch,"It is the year {c%d{x.\n\r\n\r", time_info.year);
	...
	if(double_xp_ticks != 0)  {
		if(double_xp_ticks == -1) 
			...
			Cprintf(ch, "{gDouble Experience is turned on for %d ticks!{x\n\r", double_xp_ticks);
	...
	if(double_qp_ticks != 0)  {
		if(double_qp_ticks == -1) 
			...
			Cprintf(ch, "{cDouble Quest Points is turned on for %d ticks!{x\n\r", double_qp_ticks);
...
/* Roll command added by Aql, Apr 25 2009 */

void
do_roll(CHAR_DATA * ch, char *argument)
{
	...
	{
		Cprintf(ch, "Roll syntax is \"roll <number> <sides>\": e.g. \"roll 1 6\"\n\r");
		return;
	}
	
	if(atoi(arg1) < 1 || atoi(arg1) > 10 || atoi(arg2) < 1 || atoi(arg2) > 100)
	{
		Cprintf(ch, "You can roll at most 10 dice with at most 100 sides.\n\r");
		return;
	}
	...
	sprintf(buf1,"{MROLL{x: %s rolled %i %i-sided %s for a result of %i.",ch->name,number,sides,number == 1 ? "die" : "dice",result);
	sprintf(buf2,"{MROLL{x: You rolled %i %i-sided %s for a result of %i.",number,sides,number == 1 ? "die" : "dice",result);
...
char_line(CHAR_DATA * ch, CHAR_DATA * looker, char *text) {
    ...
    char shDesc[MAX_INPUT_LENGTH] = "                    ";	/* either level/race/class or immTitle */
    char leadbuf[MAX_INPUT_LENGTH];
    char rpbuf[MAX_INPUT_LENGTH];
    char *invisFlag, *hideFlag, *occularyFlag;
    char buf[MAX_STRING_LENGTH];
    char raceName[MAX_INPUT_LENGTH];
    ...
    if (ch->level < 51 || (ch->level == 51 && ch->trust != 52)) /* Non-remort/reclass case */
    {
        sprintf(shDesc, "%2d %7s %7s", ch->level, raceName, className);
    }
    else if (ch->trust == 52 && ch->level == 51) /* Recruiter case */
    {
        /* Old code to show "Recruiter" whodesc for 51 recruiters */
    ...
    else if (ch->trust == 53 && ch->level == 52) /* Leader case */
    {
	if (ch->clan->loner)
	    sprintf(shDesc, "%18s", independentLeadership);
	else
    ...
    /* New-school recruiter tag, as well as non-51 leader tags */
    
    if (ch->trust == 52)
    {
	    sprintf(leadbuf, "[{cRecruiter{x] ");
    }
    else if (ch->trust == 53 && ch->level < 51)
    {
	    sprintf(leadbuf, "[{cLeader{x] ");
    }
    else
    {
	    leadbuf[0] = '\0';
    ...
    sprintf(buf, "{x[%s] %s%s%s%s%s%s%s%s{x%s%s%s%s%s%s%s%s%s%s%s%s%s{x",
                    shDesc,
                    wiziShow,
                    incogShow,
                    invisFlag,
...
do_who(CHAR_DATA * ch, char *argument) {
    ...
    for (iClan = clan_list; iClan != NULL; iClan = iClan->next )
    {
        rgfClan[clancount] = FALSE;
	clancount++;
    }
        ...
        if (is_number(arg)) {
			...
			if (iRace == 0 || iRace >= MAX_PC_RACE)
			{
				...
				else
				{
					...
					if (str_cmp(arg,"noclan")
						&& iClan == clan_lookup("noclan")) //they did like "who blargh"
							iClan = NULL;
						
					if (iClan)
					{
						fClanRestrict = TRUE;
						rgfClan[iClan->vnum] = TRUE;
					}
					else
					{
	...
	rgfRace[race_lookup("blue dragon")] =
	rgfRace[race_lookup("platinum dragon")] =
	rgfRace[race_lookup("gold dragon")] =
	rgfRace[race_lookup("silver dragon")] =
	rgfRace[race_lookup("copper dragon")] =
	rgfRace[race_lookup("iron dragon")] = TRUE;

        fRaceRestrict = TRUE;
    }

    for (continent = 0; continent < 2; continent++) {
	extern int hunt_continent;
	...
	if(hunt_quest_timer < 0 && hunt_continent == continent) {
		sprintf(buf, "[      TARGET      ] A {yr{Yar{ye{x and {mw{Mo{mn{Md{mr{Mo{mu{Ms{x beast\n\r");
		add_buf(output, buf);
	}
...
do_where(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0')
	{
		...
		if (ch->in_room->area->clan_control && ch->in_room->area->clan_control != clan_lookup("noclan"))
		{
			Cprintf(ch, "Controlled by: %s\r\n", capitalize(ch->in_room->area->clan_control->name));
		}
		...
		for (victim = char_list; victim != NULL; victim = victim->next)
		{
			...
			{				
				//if a hermit hiding in their hideout, go to next victim
				if ((paf = affect_find(victim->affected, gsn_hideout)) != NULL) {
					if (paf->modifier == victim->in_room->vnum) {
						continue;
					} 
				}
				found = TRUE;
...
do_practice(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0')
	{
		...
		for (sn = 0; sn < MAX_SKILL; sn++)
		{
			...
			// Don't show illegal skills in prac list [by Aql, August 2018]
			if (!is_legal_skill(ch, sn))
				continue;
			
			// Tsongas 2/21/17: Skill not known, unless it is stolen or learned by tattoo!
			if (tattoo_found == FALSE &&
				spell_steal_sn != sn &&
			...
			{
				// Tsongas 2/21/17: add color to stolen spells for easy identification
				if(spell_steal_sn == sn || tattoo_found == TRUE)
					sprintf(buf, "%s{C%-18s %3d%%  {x", buf, skill_table[sn].name, ch->pcdata->learned[sn]);
				else
	...
	else
	{
		...
		if ( sn < 0 || !is_legal_skill(ch, sn) || 
			(!IS_NPC(ch) && (ch->level < skill_table[sn].skill_level[ch->charClass] || ch->pcdata->learned[sn] < 1 /* skill is not known */ )))
		{
...
do_password(CHAR_DATA * ch, char *argument)
{
	...
	*pArg = '\0';
	 ...
	 */
	pwdnew = (char *) long_crypt(arg2, ch->name);
	for (p = pwdnew; *p != '\0'; p++)
	{
		if (*p == '~')
		{
...
lore_obj(CHAR_DATA *ch, OBJ_DATA *obj) {
	...
	if (IS_SET(obj->wear_flags, ITEM_NO_DRAGON)) {
		Cprintf(ch, "Item cannot be used by dragons.\n\r");
	}
	...
	case ITEM_AMMO:
		...
		//ammo weight is now defined in merc.h, adjust if needed
			obj->weight / AMMO_WEIGHT );
		...
		break;
	case ITEM_CHARM:
		Cprintf(ch, "Item is used with ninjutsu skill.\n\r");
		Cprintf(ch, "Damage is %dd%d (average %d).\n\r",
			obj->value[1], obj->value[2],
 			(1 + obj->value[2]) * obj->value[1] / 2);
		Cprintf(ch, "ninjutsu effect: %s\n\r", flag_string(vuln_flags, obj->value[4]));
	...
	for (paf = obj->affected; paf != NULL; paf = paf->next)
		...
		if (paf->bitvector)
		{
			...
			Cprintf(ch, "%s ", skill_table[paf->type].name);
				...
				flag_string( imm_flags, paf->bitvector)); break;
                        case TO_RESIST:
                                Cprintf(ch, "adds resistance to %s",
	 			flag_string( res_flags, paf->bitvector)); break;
			case TO_VULN:
                                Cprintf(ch, "adds vulnerability to %s",
				flag_string( vuln_flags, paf->bitvector)); break;
                        default:
	...
	//usion once we have the new spells that casters cast, we should add those gsn types here
	...
	if (obj->pIndexData->vnum == 6725 && obj->timer > 0) //6725 == generic tattoo item
		Cprintf(ch,"The tattoo will last for %d more hours.\n\r",obj->timer);
