...
void note_editor_swap(CHAR_DATA *ch, char *argument);
/*
...
// 1-based
char*
getBoardName(int boardNumber) {
    return getBoard(boardNumber)->name;
}

BOARD_DATA *
getClanBoard(CLAN_DATA *clan) {
	BOARD_DATA *pBoard;
	if (clan == NULL) {
		return NULL;
	}
	for (pBoard = board_first; pBoard != NULL; pBoard = pBoard->next) {
		if (pBoard->clan != NULL && !str_cmp(pBoard->clan, clan->name)) {
			return pBoard;
		}
	}
	return NULL;
...
canAccess(CHAR_DATA *ch, BOARD_DATA *board) {
    ...
    if (board->clan
            && str_cmp(board->clan, ch->clan->name)
            && get_trust(ch) < 55 ) 
    {
        return FALSE;
    }
...
do_nwrite (CHAR_DATA *ch, char *argument) {
    ...
    if (ch->pcdata->in_progress 
            && (strlen(ch->pcdata->in_progress->text) == 0
            || !ch->pcdata->in_progress->subject
            || !ch->pcdata->in_progress->to_list)) { // Fixed note write crash bug [by Aql, September 2018]
...
do_note (CHAR_DATA *ch, char *argument) {
	...
	} else if (!str_prefix (arg, "editor")) {
		note_editor_swap(ch, argument);
	} else {
...
note_reply(CHAR_DATA *ch, char *argument) {
    ...
    char temp_to[MAX_INPUT_LENGTH] = "\0";
    // Used to store new, edited to list (compact)
    char to[MAX_INPUT_LENGTH] = "\0";
    // Used to store ctime
    char *strtime;
    ...
    /*set to listing*/
    /*Add sender and arguments to the to list*/
    sprintf(temp_to, "%s %s", p->sender, argument);
    /*If first argument is cc add replied note recipients to the to list*/
    if (!str_prefix("cc", argument)){
    	sprintf(temp_to, "%s %s %s", temp_to, p->to_list, p->sender);
    }
    /*Parse list to remove duplicates and cc*/
    tp = temp_to;
    do {
    	tp = one_argument(tp, arg1);
    	/*Break loop when no args left*/
    	if (arg1[0] == '\0') {
    		break;
    	//Cannot sent to all on personal
    	}else if (strcasecmp(ch->pcdata->board->name, "personal") == 0 && strcasecmp(arg1, "all") == 0){
    		sprintf(temp_to, "%s %s %s", temp_to, p->to_list, p->sender);
    		Cprintf(ch, "You cannot send a note to all on the personal board.\n\r");
    		Cprintf(ch, "All original senders and recipients added as recipients. Please use CC to do this in the future.\n\r");
    		continue;
    	/*skip duplicates*/
    	}else if(is_name(arg1, to) || strcasecmp(arg1, "cc") == 0) {
    		continue;
    	/*add to list*/
    	}else{
...
handle_con_note_subject (CHAR_DATA *ch, char * argument) {
    ...
    if (!buf[0]) {
        ...
        if (IS_IMMORTAL(ch)) {
			...
			ch->pcdata->in_progress->text = str_dup("");
            if (ch->pcdata->note_editor == OLD_NOTE_EDITOR) {
				Cprintf(ch, "\n\r");
				Cprintf(ch, "Enter text. Type END on an empty line to end note.\n\r");
				Cprintf(ch, "================================================================================\n\r");
				ch->desc->connected = CON_NOTE_TEXT;
...
void
note_editor_swap(CHAR_DATA *ch, char *argument) {

	if (ch->pcdata->note_editor == OLD_NOTE_EDITOR) {
		ch->pcdata->note_editor = NOTE_STRING_EDITOR;
		Cprintf(ch, "You will now use the new note editor!\n\r");
	} else {
		ch->pcdata->note_editor = OLD_NOTE_EDITOR;
		Cprintf(ch, "You will now use the classic note editor.\n\r");
	}
