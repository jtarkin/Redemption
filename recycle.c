...
new_char(void)
{
	...
	*ch = ch_zero;
	...
	ch->shop_discount[0] = &str_empty[0];
	ch->shop_discount[1] = &str_empty[0];
	ch->shop_mob_vnum = 0;
	ch->logon = current_time;
	ch->lines = PAGELEN;
	ch->clan = clan_lookup("noclan");
	for (i = 0; i < 4; i++)
		ch->armor[i] = 100;
...
free_char(CHAR_DATA * ch)
{
	...
	free_string(ch->shop_discount[0]);
	free_string(ch->shop_discount[1]);
...
new_pcdata(void)
{
	...
	*pcdata = pcdata_zero;
	...
	pcdata->shaper_penalty = 0;
	pcdata->macroInstances = NULL;
	pcdata->macroTimer = 0;
	pcdata->buffer = new_buf();

    // Default to level 1 in all classes.
    for(i=0; i<MAX_CLASS; i++) {
        pcdata->class_progression[i].class_name = class_table[i].name;
        pcdata->class_progression[i].reclass = 0;
        pcdata->class_progression[i].level = 0;
        pcdata->class_progression[i].exp_total = 0;
        pcdata->class_progression[i].cp = 0;
        pcdata->class_progression[i].trains = 0;
        pcdata->class_progression[i].practices = 0;
        pcdata->class_progression[i].hp = 0;
        pcdata->class_progression[i].mana = 0;
        pcdata->class_progression[i].move = 0;
		pcdata->class_progression[i].strength = 1;
		pcdata->class_progression[i].intelligence = 1;
		pcdata->class_progression[i].wisdom = 1;
		pcdata->class_progression[i].dexterity = 1;
		pcdata->class_progression[i].constitution = 1;

        for(j = 0; j < MAX_GROUP; j++) {
            pcdata->class_progression[i].group_known[j] = FALSE;
        }
    }
...
/*    pArea->recall           =   ROOM_VNUM_TEMPLE;      ROM OLC */
    ...
    pArea->max_vnum = 0;
    pArea->age = 0;
    pArea->clan_control = clan_lookup("noclan");
    pArea->nplayer = 0;
    pArea->empty = TRUE;        /* ROM patch */
    pArea->autocontrol = TRUE;
...
CLAN_DATA *new_clan(void)
{
	...
	if (clan_free == NULL)
		clan = (CLAN_DATA *) alloc_perm(sizeof(*clan));
	else
	{
		clan = clan_free;
	...
	VALIDATE(clan);
	clan->next = NULL;
	clan->name = str_dup("noclan");
	clan->who_name = str_dup("[NULLCLAN]");
	clan->leader = str_dup("none");
	clan->recruiter1 = str_dup("none");
	clan->recruiter2 = str_dup("none");
	clan->mostkilled = str_dup("none");
	clan->mostkills = str_dup("none");
	clan->loner = FALSE;
	clan->clanner = FALSE;
	clan->spawn_terra = ROOM_VNUM_ALTAR;
	clan->spawn_dominia = 31001;
	clan->clanpoints = 0;
	clan->clangold = 0;
	clan->upkeep_points = 0;
	clan->upkeep_gold = 0;
	clan->tempkills = 0;
	clan->tempdeaths = 0;
	clan->pkills = 0;
...
void free_clan(CLAN_DATA *clan)
{
	...
	free_string(clan->recruiter1);
	free_string(clan->recruiter2);
...
/* stuff for recycling mvars */
MOB_VAR * mvar_free;

MOB_VAR *
new_mvar(void) {
    static MOB_VAR mvar_zero;
    MOB_VAR *mvar;

    if (mvar_free == NULL) {
        mvar = (MOB_VAR *) alloc_perm(sizeof(*mvar));
    } else {
        mvar = mvar_free;
        mvar_free = mvar_free->next;
    }

    *mvar = mvar_zero;
	mvar->target_name = &str_empty[0];
	mvar->varname = &str_empty[0];
	mvar->value = 0;
...
HELP_DATA* new_help_data( void )
{
    ...
    pHelp = malloc( sizeof( HELP_DATA ) );
    memset( pHelp, 0, sizeof( HELP_DATA ) );
