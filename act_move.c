...
move_char(CHAR_DATA * ch, int door, bool follow)
{
	...
	if(number_percent() - 8 > get_skill(ch, gsn_sneak)
	...
	if (is_affected(ch,gsn_backstab) && number_percent() < 40) //rogue crippling strike
	{
		Cprintf(ch, "Your injured leg buckles beneath you!\n\r");
		act("$n buckles under $s injured leg!", ch, NULL, NULL, TO_ROOM, POS_RESTING);
		WAIT_STATE(ch, 4);
		return;
	}
	
	// just like crippled heel, except by arrows
	if (is_affected(ch,gsn_pindown))
	{
		paf = affect_find(ch->affected, gsn_pindown);
		if ( paf
//		  && paf->modifier == 1 // this appears pointless at this time.
		  && number_percent() < 50 )
		{
			Cprintf(ch, "You stumble due to the arrow stuck in you!\n\r");
			act("$n buckles under $s injured leg!", ch, NULL, NULL, TO_ROOM, POS_RESTING);
			//WAIT_STATE(ch, PULSE_VIOLENCE); //this was too much, so it was removed
// this looks like it prevents movement out of a room by exiting here.
			return;
		}
	}
	...
	if((paf = affect_find(ch->affected, gsn_bleeder)) != NULL) {
		...
		leave_bloodpool(ch, door);
		
		if(paf->modifier <= 0) {
			Cprintf(ch, "Your wounds close and you stop leaving a blood trail.\n\r");
			affect_strip(ch, gsn_bleeder);
		}
	}
	...
	//Deal with NCs/Clanners - Teldin
	if (to_room->clan != clan_lookup("noclan") &&
		!IS_IMMORTAL(ch) &&
		to_room->clan->clanner != ch->clan->clanner)
	{
		//clanners can enter each others rooms, nonclanners can't
		Cprintf(ch, "You cannot enter that room.\n\r");
		return;
	}
	if (to_room->clan != clan_lookup("noclan") &&
		!IS_IMMORTAL(ch) &&
		!to_room->clan->clanner &&
		ch->clan != to_room->clan)
	{
		//nonclanners cannot enter each others guild halls
		Cprintf(ch, "You cannot enter that room.\n\r");
		return;		
	}
	...
	if (!IS_NPC(ch))
	{
	    ...
	    if (in_room->sector_type == SECT_UNDERWATER || room_is_affected(ch->in_room, gsn_flood)) {
	        ...
	        if (affect != NULL) {
	            if (!ch->clan->clanner) {
	                ...
	                ignoreFlood = TRUE;
	            } else if (ch->clan->clanner && !affect->clan) {
		...
		if(room_is_affected(ch->in_room->exit[door]->u1.to_room, gsn_repel)) {
			...
			{
				Cprintf(ch,"{cYour defensive posture slows you down!{x\n\r");
				WAIT_STATE(ch, 6);
			}			
			else if(!is_affected(ch, gsn_shadow_walk))
				WAIT_STATE(ch, 1);
...
do_open(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_AFFECTED(ch, AFF_HIDE) && ch->reclass != reclass_lookup("rogue"))
	{
		REMOVE_BIT(ch->affected_by, AFF_HIDE);
	}
...
void do_roar(CHAR_DATA *ch, char *argument)
{
	...
	{
		Cprintf(ch, "You open your mouth to ROAR and... *blub blub blub*\n\r");
		act("$n opens $s mouth to ROAR and... *blub blub blub*", ch, NULL, NULL, TO_ROOM, POS_RESTING);
	}
	else
	{
...
do_recall(CHAR_DATA * ch, char *argument)
{
	...
	if (is_affected(ch, gsn_beacon) && number_percent() < 85)
	{
		if (! (is_affected(ch, gsn_gullivers_travel) &&
	...
	//60% chance of losing no moves for recalling
	if (gulliver_keep_mv == 0)
	{
		if (check_toolbelt(ch) == 6 && number_percent() < 80)
		{
			Cprintf(ch,"{yYour glass hummingbird chimes gently in your ear.{x\n\r");
			act("A tiny glass hummingbird appears next to $n and emits a musical chime.", ch, NULL, NULL, TO_ROOM, POS_RESTING);
		}
		else
			ch->move /= 2;
	}
...
do_train(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp("sliver", argument))
	{
		...
		while (slivtype == ch->sliver)
		{
			...
			if (ch->sliver > MAX_SLIVER)
				ch->sliver = MAX_SLIVER;
		}
...
void do_ensnare(CHAR_DATA *ch, char *argument)
{
	...
	chance = get_skill(ch, gsn_ensnare) * 6 / 7 ;
	...
	{
	case 1:
		Cprintf(ch, "You set up a concealed poison needle trap.\n\r" ); break;
	case 2:
		Cprintf(ch, "You set up a trap to fire a barrage of darts.\n\r"); break;
	case 3:
		Cprintf(ch, "You set up a concealed trip wire to take someone down.\n\r" ); break;
	default:
