...
affect_modify(CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd) {
    ...
    if ((wield = get_eq_char(ch, WEAR_WIELD)) != NULL
		&& (wield->weight / 20 + 1 > get_curr_stat(ch, STAT_STR) 
			|| get_curr_stat(ch,STAT_STR) <= 3 )
		&& ch->in_room != NULL)
    {
            if (!obj_is_affected(wield, gsn_graft)) {
				...
				if (paf->location == APPLY_STR) {
					act("$p is too heavy for your arm, and tears at your skin and bone\n\r", ch, wield, NULL, TO_CHAR, POS_RESTING);
					act("$n's arm suddenly plummets to the floor, dragged down by the weight of $p.  You hear skin and bone wrench.\n\r", ch, wield, NULL, TO_ROOM, POS_RESTING); 
					damage(ch, ch, dice(ch->level / 7, 4), gsn_graft, DAM_HARM, TRUE, TYPE_MAGIC);
				}
			}	
    }
...
affect_to_char(CHAR_DATA * ch, AFFECT_DATA * paf) {
    ...
    *paf_new = *paf;
    ...
    if (ch->race == race_lookup("dwarf")
	    ...
	    && strcmp(skill_table[paf_new->type].name,"enlargement")) { // Ancestry won't affect enlargement
        found = 0;
        new_duration = paf_new->duration / 2;
        paf_new->duration = new_duration;
    }

    if (ch->race == race_lookup("dwarf") && ch->remort)
    {
        // Fix ALL affects of this type (bleh)
        for (caf = ch->affected; caf != NULL; caf = caf->next) {
            if (caf != paf_new
                    && caf->type == paf_new->type
		    && caf->duration != paf_new->duration) {

		if (paf_new->duration > caf->duration)
		{
                    found = 1;
		}
                caf->duration = paf_new->duration;
    ...
    if (found == 1) {
    	Cprintf(ch, "You overcome your ancestral heritage.\n\r");
    }
...
void affect_v_to_obj( OBJ_DATA* obj, AFFECT_DATA* paf)
{
    AFFECT_DATA* paf_new;
    ...
    *paf_new = *paf;
    VALIDATE(paf_new);
    paf_new->next = obj->affected_v;
    obj->affected_v = paf_new;
...
affect_remove(CHAR_DATA * ch, AFFECT_DATA * paf) {
    ...
    //to stop people from dying to remove cooldown prematurely 
...
void affect_v_remove_obj( OBJ_DATA* obj, AFFECT_DATA* paf )
{
    int where, vector;

    if (obj->affected_v == NULL)
    {
        bug("Affect_v_remove_object: no affect.", 0);
        return;
    }
    ...
    if ( paf == obj->affected_v )
        obj->affected_v = paf->next;
    else
    {
        AFFECT_DATA* prev;

        for ( prev = obj->affected_v; prev; prev = prev->next )
            if (prev->next == paf)
	    {
                prev->next = paf->next;
                break;
        ...
        if ( !prev )
	{
            bug( "Affect_v_remove_object: cannot find paf.", 0 );
            return;
        }
    }
...
bool obj_is_affected_v( OBJ_DATA* obj, int sn )
{
    return affect_find( obj->affected_v, sn ) != NULL;
}
...
affect_join(CHAR_DATA * ch, AFFECT_DATA * paf) {
    AFFECT_DATA *paf_old;
	int modifier_cap, duration_cap;
	bool capped = FALSE;
	
	// Currently capping four spells: fear, plague, poison, and psionic psychic crush (see affect_merge)
	if(!IS_NPC(ch)) {
		if(paf->type == skill_lookup("plague")) {
			modifier_cap = -15;
			duration_cap = 20;
			capped = TRUE;
		}
		if(paf->type == skill_lookup("poison")) {
			modifier_cap = -10;
			duration_cap = 20;
			capped = TRUE;
		}
		if(paf->type == skill_lookup("fear")) {
			modifier_cap = -15;
			duration_cap = 20;
			capped = TRUE;
		}
    ...
    for (paf_old = ch->affected; paf_old != NULL; paf_old = paf_old->next) {
		...
		if(capped) {
			if(paf->duration > duration_cap)
				paf->duration = duration_cap;
			if(paf->modifier > 0 && paf->modifier > modifier_cap)
				paf->modifier = modifier_cap;
			if(paf->modifier < 0 && paf->modifier < modifier_cap)
				paf->modifier = modifier_cap;
		}
...
affect_merge(CHAR_DATA * ch, AFFECT_DATA * paf, int sn) {
    ...
    for (paf_old = ch->affected; paf_old != NULL; paf_old = paf_old_next) {
		...
		paf->modifier += paf_old->modifier;
		// Multi-location effects that need to be capped go here
			if(paf->type == skill_lookup("psychic crush")) { 
				if(paf->location == APPLY_SAVING_SPELL && paf->modifier > 10)
					paf->modifier = 10;
				else if(paf->location == APPLY_INT && paf->modifier < -5)
					paf->modifier = -5;
				else if(paf->location == APPLY_WIS && paf->modifier < -5)
					paf->modifier = -5;
			}
            affect_remove(ch, paf_old);
            break;
